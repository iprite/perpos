...
## 4.6 หน้าภาพรวมสำหรับ "เจ้าของกิจการ" (Phase 2)

KPI เดิมของหน้า `/accounting` เป็น **เงินสดจริง** (คิดจาก `acc_entries`) → org ที่ออกใบกำกับแล้วแต่ยัง
ไม่เก็บเงินเห็น 0 ทั้งแถว เจ้าของกิจการเลยงงว่า "ขายได้แล้วเงินอยู่ไหน"

บล็อกบนสุดของหน้าจึงตอบจาก **เอกสารขาย** แทน (วางไว้ก่อนการ์ดเงินสด):

- **ยอดขายเดือนนี้ (ตามเอกสาร)** · **รอเก็บเงิน** · **เลยกำหนดชำระ** — ทั้งหมดคิดผ่าน
  `selectBillingDocuments()` + `billingSign()` (กฎเดียวกับ auto journal → ไม่นับซ้ำทั้งสายเอกสาร)
- **"ต้องตามต่อ"** — เอกสารที่เลยกำหนด + ที่ครบกำหนดใน 7 วัน เรียงตามความเร่งด่วน สูงสุด 5 รายการ
  (ตอบคำถามเดียวที่เจ้าของถามทุกเช้า: วันนี้ต้องตามใคร) · ว่าง = empty state พร้อม CTA ไปหน้าเอกสารขาย
- การ์ดเงินสด/กำไร/กราฟ/ภาษี ยังอยู่ครบด้านล่าง (นักบัญชียังใช้เหมือนเดิม)
