...
> **แก้แล้ว 2026-08-18 — 2 blocker ของหน้าคลัง (`/[orgSlug]/just-me/inventory`)**
>
> 1. **GET เคยตัด 1,000 แถวเงียบ** (`items`/`balances`/`serials`/`costs`/`cost_monthly` ยิงครั้งเดียว = เพดาน PostgREST)
>    ⇒ org ที่มี serial/เศษสายไฟเกินพันเส้น ยอดคงเหลือ+มูลค่าคลังต่ำกว่าจริงโดยไม่มีสัญญาณใด ๆ ·
>    ตอนนี้ `fetchAll()` ใน [api/just-me/inventory/route.ts](../apps/perpos/src/app/api/just-me/inventory/route.ts)
>    ไล่ทีละ 1,000 จนครบ (เพดานกัน runaway 20,000) เรียงด้วยคีย์ที่ไม่ซ้ำเสมอ และคืนธง `listsTruncated`
>    ให้หน้าเว็บขึ้นแถบแดงเมื่อชนเพดาน — **ห้ามเปลี่ยนกลับไปเรียกครั้งเดียว**
> 2. **แก้/ลบทะเบียนไม่ได้เลย** (มีแต่ create) ⇒ ชื่อ-รหัสพิมพ์ผิด และวัสดุที่ OCR สร้างอัตโนมัติจากบิลค้างถาวร ·
>    เพิ่ม action `update_warehouse`/`delete_warehouse`/`update_item`/`delete_item` + คลิกแถวเพื่อแก้ในหน้าเว็บ ·
>    **invariant ใหม่**: FK ของ `stock_movements`/`balances`/`serials` เป็น `ON DELETE CASCADE`
>    ⇒ **ลบได้เฉพาะแถวที่ยังไม่ถูกอ้างที่ไหนเลย** (เช็คก่อนลบทุกครั้ง) นอกนั้นต้อง **ปิดใช้งาน**
>    (`is_active` — วัสดุเพิ่งได้คอลัมน์นี้ใน migration `20260818090000_just_me_item_is_active`) ·
>    ปิดใช้งาน = หายจากช่องเลือกตอนเบิก-รับ แต่ประวัติ/ยอด/ต้นทุนอยู่ครบ ·
>    สลับวิธีติดตาม (Serial/ตัดเมตร) ได้เฉพาะวัสดุที่ยังไม่เคยเดินรายการ
>
> **แก้แล้ว 2026-08-18 — สแกนบิลรับของ (`inventory/ocr`) อ่านบิลยาวไม่ได้**
>
> เดิม route ยิง Gemini เองตรง ๆ ด้วย `maxOutputTokens: 2000` **โดยไม่ปิด thinking** —
> gemini-2.5-flash กิน thinking token 1,500–1,900 จากโควตาเดียวกัน ⇒ บิล ~13 รายการ
> `finishReason: MAX_TOKENS` → JSON ขาด → ผู้ใช้เจอ "ไม่สามารถอ่านบิลได้" **ทุกครั้ง**
> (บิล 5 รายการเหลือ margin ~230 token จึงหลุดเป็นระยะ) · และไม่มี `maxDuration`
> ทั้งที่ใช้เวลา 8–16 วิ ⇒ บน Vercel (default 10 วิ) จบด้วย 504
>
> ตอนนี้เรียกผ่าน **`aiChat`** ([lib/ai/client.ts](../apps/perpos/src/lib/ai/client.ts)) ตามมาตรฐานเดียวของระบบ —
> ได้ `thinkingBudget:0` + `jsonMode` + นับต้นทุนอัตโนมัติมาในตัว (`maxTokens: 8000`, `maxDuration = 60`) ·
> **`aiChat` รับรูปได้แล้ว** ผ่าน `opts.images` (Gemini เท่านั้น — provider อื่นจะคืน `null` ไม่ปล่อยผ่านเงียบ)
> ⇒ วัดหลังแก้: บิล 14 รายการ อ่านสำเร็จ 3/3 · รูปถ่ายเอียง 2/2 · เวลา 3.5–7.7 วิ (เดิม 8–16 วิ) ·
> **ห้ามกลับไป `fetch` Gemini ตรงในเส้นทางนี้อีก**
>
> ที่ยังไม่ได้แก้ (รู้แล้ว): ค่าจัดส่ง/ค่าบริการยังหลุดเข้ามาเป็นรายการวัสดุ ต้องลบเอง ·
> ชื่อต่างกันแค่เว้นวรรค = สร้างวัสดุใหม่ (จับคู่ด้วยชื่อตรงเป๊ะเท่านั้น) ·
> วันที่บิลที่ AI อ่านได้ถูกทิ้ง (กล่องรับของไม่ใช้) และอ่านปีผิดในรูปถ่ายบางใบ
