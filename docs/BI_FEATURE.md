...
**metric ที่ยัง `draft` ตอบไม่ได้ — แต่ต้อง "บอกว่ามีอยู่"** (พฤติกรรมที่ถูกต้อง ไม่ใช่บั๊ก)

ถ้าคำถามตรงกับ metric ร่าง บอทตอบว่า _"ระบบมีตัวชี้วัดอยู่ (ชื่อ…) แต่ยังไม่ได้ยืนยันนิยาม จึงยังตอบเป็นตัวเลขให้ไม่ได้"_ พร้อมนิยามที่ร่างไว้ — **ไม่ใช่** "ธุรกิจไม่มีข้อมูล" ซึ่งคนละเรื่องกันและทำลายความเชื่อถือ · **ห้ามรัน metric ร่างเด็ดขาด** (payload คืน `metric=null`, `rows=[]`)

การตัดสินว่า "ตรงกับ metric ร่างไหม" ต้องผ่าน **สองด่านพร้อมกัน** — `cosine(embedding คำถาม, embedding metric) ≥ DRAFT_MIN_SIMILARITY` **และ** มีคำตรงกัน (`matchByKeyword`) · เหตุผล: keyword อย่างเดียวเอาคำหน้าที่สั้น ๆ ในภาษาไทยไม่อยู่ (token "ยัง" ใน "วันนี้เป็นยังไงบ้าง" ไปตรงกับคำพ้อง "ยังไม่จ่าย") · **draft ที่ยังไม่ได้ `pnpm bi:embed` จะไม่ถูกเปิดเผยเลย (fail-safe)** — ปล่อยผ่านด้วย keyword อย่างเดียวเท่ากับบั๊กเดิมกลับมาเงียบ ๆ

### 3.2.1 ค่าคงที่ที่ปรับได้ — **วัดจากข้อมูลจริง ห้ามปรับด้วยความรู้สึก**

| ค่า                        | ที่อยู่                                                | ปัจจุบัน | ฐานที่ใช้ตั้ง                                                                                                                                                                      |
| -------------------------- | ------------------------------------------------------ | -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `MIN_SIMILARITY`           | [`resolver.ts`](../apps/perpos/src/lib/bi/resolver.ts) | 0.60     | เกณฑ์รับ candidate เข้าพิจารณา                                                                                                                                                     |
| `SUGGEST_MIN_SIMILARITY`   | `resolver.ts`                                          | 0.68     | เกณฑ์ที่จะ "เอ่ยชื่อ metric" เป็นข้อเสนอ (ต่ำกว่านี้ = ไม่เสนอ ดีกว่าเสนอมั่ว)                                                                                                     |
| `DECISIVE_GAP`             | `resolver.ts`                                          | 0.05     | อันดับ 1 นำห่างเท่าไรถึงตอบได้โดยไม่ถามกลับ (ใช้คู่กับ "LLM เลือกตรงกัน + confidence ≥ 0.6")                                                                                       |
| **`DRAFT_MIN_SIMILARITY`** | [`metrics.ts`](../apps/perpos/src/lib/bi/metrics.ts)   | **0.64** | **วัดบน prod 2026-07-24**: คำถามที่ควรเจอ 0.664–0.753 · small-talk ที่ห้ามเจอ 0.569–0.614 → ตั้งกลางช่องว่าง · **ค่า 0.70 ตามสัญชาตญาณจะตัดคำถามกำไรที่ถูกต้องทิ้ง 2 ใน 4 ประโยค** |

วิธีวัดใหม่เมื่อข้อมูล/นิยามเปลี่ยน: embed คำถามชุด "ควรเจอ" และ "ห้ามเจอ" เทียบกับ `bi_metrics.embedding` บน prod แล้วดูช่องว่างจริง — อย่าปรับจากความรู้สึกว่า "น่าจะเข้มไป/หลวมไป"
...
### 9.7 🗺️ Coverage map — scope `tmc` (2026-07-25)

**ทำไมต้องมี:** metric ที่ผ่านมาเกิดแบบ ad-hoc (นึกออกก็เขียน) ⇒ ไม่มีใครตอบได้ว่า "ธุรกิจนี้เหลืออะไรที่ถามไม่ได้" · แผนที่นี้แตกจาก **กระบวนการธุรกิจ ไม่ใช่จากตาราง** แล้วชี้ว่าแต่ละช่องว่างติดที่อะไร — ใช้คู่กับ [`_bi_review_log.sql`](../supabase/migrations/_bi_review_log.sql) (บนลงล่าง + ล่างขึ้นบน มีทิศเดียวไม่มีวันครอบคลุม)

**ฐานข้อเท็จจริง** (org `tmc` `1f52618c-…`, นับ 2026-07-25): stays 203 · แขกไม่ซ้ำ 194 (ซ้ำ 8 คน = 4.1%) · finance 701 · petty 561 · properties 7 · stock_items 41 · investments 6 (รวม 118.0 ล้าน) · guests 201 · **stock_movements 0**

| #   | กระบวนการ                       | คำถามที่เจ้าของถามซ้ำ                         | สถานะ                                                                   | ติดอะไร / ทำต่ออย่างไร                                                                                                                                                                                        |
| --- | ------------------------------- | --------------------------------------------- | ----------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | เข้าพัก — ปริมาณ                | เดือนนี้กี่ครั้ง กี่คืน แยกแปลง/ช่องทาง       | ✅ `stay_count` `stay_nights` `stay_avg_nights`                         | —                                                                                                                                                                                                             |
| 2   | เข้าพัก — รายได้                | ค่าห้อง/อาหารเท่าไร ADR เท่าไร                | ✅ `stay_room_revenue` `stay_fnb_revenue` `stay_adr`                    | —                                                                                                                                                                                                             |
| 3   | **อัตราการเข้าพัก (occupancy)** | **"แปลงไหนว่างเยอะ"**                         | 🔴 **ทำไม่ได้**                                                         | **`tmc_properties` ไม่มีจำนวนห้อง/ความจุ** ⇒ ไม่มีตัวหาร · KPI อันดับ 1 ของธุรกิจที่พักและเป็นช่องว่างที่ใหญ่ที่สุด — ต้องเพิ่มฟิลด์ความจุที่โมดูล `tmc` ก่อน (งานฝั่ง product ไม่ใช่ฝั่ง BI)                 |
| 4   | รายได้ต่อหัว                    | รับกี่คน ต่อหัวเท่าไร                         | 🟡 ทำได้แต่ต้องประกาศความครอบคลุม                                       | `group_size` กรอก 165/203 (81%) — ถ้าไม่บอกจำนวนใบที่คิดได้ จะซ้ำรอยกับดัก "การ์ดต้นทุนบอกจำนวนใบผิด" (§7)                                                                                                    |
| 5   | ลูกค้ากลับซ้ำ                   | แขกเก่ากลับมากี่ %                            | 🟡 ทำได้ ฐานยังเล็ก                                                     | `guest_id` ครบ 203/203 · ซ้ำแค่ 8 คน — เขียนได้เลยแต่ตัวเลขยังไม่มีนัยจนกว่าจะสะสมอีกหลายเดือน                                                                                                                |
| 6   | ส่วนลด/โปรโมชัน                 | ให้ส่วนลดไปเท่าไร คุ้มไหม                     | 🟡 ทำได้ ฐานเล็ก                                                        | `promotion_pct > 0` แค่ 18/203                                                                                                                                                                                |
| 7   | **คุณภาพ/ปัญหา**                | **"เดือนนี้มีเรื่องร้องเรียนกี่เคส แปลงไหน"** | 🔴 ไม่มี metric                                                         | `issues` ไม่ว่าง 42 ใบ · `damaged_items` 2 · `feedback` 144 — **ข้อมูลพร้อม เขียนได้ทันที** · เป็นข้อความอิสระ ⇒ metric ต้องเป็น "จำนวนเคส" (นับ) ห้ามให้ LLM สรุปเนื้อความ (§6.4)                            |
| 8   | บริการบัตเลอร์                  | ใช้บัตเลอร์กี่ครั้ง                           | 🟡 ไม่มี metric                                                         | `butler_service_visit` 189/203 — เขียนได้ ลำดับความสำคัญต่ำ                                                                                                                                                   |
| 9   | การเงินภาพรวม                   | รับ/จ่าย/กำไร แยกแปลง/หมวด                    | ✅ `finance_income` `finance_expense` `finance_net_profit` (owner-only) | —                                                                                                                                                                                                             |
| 10  | **กำไรต่อการเข้าพัก**           | **"บ้านหลังนี้ต่อ 1 ทริปได้กำไรเท่าไร"**      | 🔴 **ทำไม่ได้**                                                         | `tmc_finance_entries.check_in_date` มีคอลัมน์แต่ **ว่างทั้ง 701 แถว** ⇒ ผูกรายจ่ายเข้ากับใบเข้าพักไม่ได้ · ปลุกเมื่อเริ่มกรอกจริง                                                                             |
| 11  | เงินสดย่อย                      | จ่ายไปเท่าไร เติมเท่าไร                       | ✅ `petty_cash_expense` `petty_cash_top_up`                             | ยอด**คงเหลือ**ยังไม่มี metric — เป็น snapshot (`time_basis=NULL`) เขียนเพิ่มได้                                                                                                                               |
| 12  | สต็อก/ของใช้                    | อะไรใกล้หมด                                   | 🟡 ไม่มี metric (snapshot)                                              | `stock_items` 41 (ตอนนี้ต่ำกว่าจุดสั่งซื้อ 0 ชิ้น) — เขียนได้ แต่เป็น snapshot ต้องระวัง BLOCKER-2 (บรรทัดนิยามห้ามโกหกเรื่องช่วงเวลา)                                                                        |
| 13  | การใช้ของ/ของหาย                | เดือนนี้ใช้ของไปเท่าไร                        | 🔴 ทำไม่ได้                                                             | **`tmc_stock_movements` 0 แถว** — ยังไม่มีใครบันทึกการเคลื่อนไหว                                                                                                                                              |
| 14  | ผลตอบแทนการลงทุนต่อแปลง         | แปลงนี้ลงทุนไป vs คืนทุนแค่ไหน                | 🟡 ไม่มี metric แต่ข้อมูลพร้อม                                          | `tmc_property_investments` 6 แถว (118.0 ล้าน, มี `annual_rate`/`starts_at`/`ends_at`) — **ต้องให้เจ้าของเซ็นนิยาม "คืนทุน" ก่อน** (กำไรสะสม? กระแสเงินสด?) นี่คือ metric ที่นิยามผิดแล้วเสียหายที่สุดในชุดนี้ |
| 15  | มัดจำ                           | มัดจำค้างคืนเท่าไร                            | ⛔ บล็อก                                                                | `deposit_received`/`deposit_returned` non-null 0/203 — `stay_deposit_outstanding` คง `draft` ตามเดิม (§9.5)                                                                                                   |
| 16  | ระยะเวลาจองล่วงหน้า             | จองล่วงหน้าเฉลี่ยกี่วัน                       | 🔴 ทำไม่ได้                                                             | ไม่มีคอลัมน์ "วันที่จอง" · `created_at` เก่าสุด 2026-05-20 = วันคีย์ข้อมูลย้อนหลัง **ไม่ใช่วันจอง** — ใช้แทนกันไม่ได้ (จะได้ตัวเลขที่ดูมีเหตุผลแต่ผิด)                                                        |
| 17  | drill-down รายการ               | "ขอดูรายตัว"                                  | 🔴 ปิดอยู่                                                              | `DETAIL_METRIC_BY_SCOPE.tmc = null` — ต้องเพิ่ม metric ระดับรายการ (`no_summarize` + `chart_hint='table'`)                                                                                                    |

**ลำดับที่ควรทำ (ตาม value ÷ effort จากข้อเท็จจริงข้างบน):**

1. **#7 คุณภาพ/ปัญหา** — ข้อมูลพร้อม 42 เคส ไม่ต้องรออะไร และเป็นคำถามที่เจ้าของกิจการที่พักถามทุกเดือน
2. **#17 metric ระดับรายการ** — ปลด drill-down ให้ทั้ง scope ทีเดียว (คำถามต่อยอด "ขอดูรายตัว" ตอนนี้ตันหมด)
3. **#11 ยอดคงเหลือเงินสดย่อย + #12 ของใกล้หมด** — snapshot คู่กัน เขียนทีเดียวจบ
4. **#14 ROI ต่อแปลง** — คุณค่าสูงสุดต่อเจ้าของ แต่ต้องเซ็นนิยามก่อน อย่ารีบ
5. **#3 occupancy** — คุณค่าสูงสุดในภาพรวม แต่ **ติดที่ product ไม่ใช่ BI** (ต้องมีความจุต่อแปลงก่อน) → ควรเปิดเป็นงานฝั่งโมดูล `tmc`
6. #4/#5/#6/#8 — เขียนได้แต่ฐานข้อมูลยังบาง รอสะสม

**ที่ไม่ต้องทำจนกว่าข้อมูลจะมา:** #10 (กำไรต่อทริป), #13 (การใช้ของ), #15 (มัดจำ), #16 (lead time) — ทั้งสี่ข้อ **ไม่ใช่ปัญหาโค้ด** และการฝืนเขียนตอนนี้จะได้ metric ที่ตอบ 0 เสมอหรือตอบเลขที่ผิด
