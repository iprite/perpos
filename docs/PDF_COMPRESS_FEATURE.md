...
| **P1g — Scheduler (ความทน)** | เพิ่ม `pdf_compress` ใน scheduler: stuck-sweep (mark failed + refund) + requeue pending + PDPA cleanup (ลบ PDF ต้นฉบับ/ผลเก่า >48ชม.) | จงใจทำ job ค้าง → scheduler เก็บกวาด + refund · ไฟล์เก่าถูกลบ | P1e |
