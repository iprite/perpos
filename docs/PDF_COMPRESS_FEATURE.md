...
const driveUrl = await Promise.race([
  saveToDrive(admin, profileId, {
    categoryKey: "pdf",
    categoryName: "เอกสาร PDF", // → "Perpos Assistant/เอกสาร PDF/"
    fileName: `${origName}-compressed.pdf`,
...
### กับดักที่เจอจริงแล้ว (2026-08-11 — PR #208/#210)

9. **ปุ่มใน LINE Flex จำกัด `uri` 1,000 ตัวอักษร — ห้ามยัด signed URL ลงปุ่มตรง ๆ**
   signed URL (~550) + `?download=<ชื่อไฟล์ไทย>` (ไทย 1 ตัว = **9 ตัวอักษร**หลัง percent-encode ·
   ชื่อ 90 ตัว → 618) = ~1,180 → LINE **ปฏิเสธทั้ง push** → `pdf/deliver` คืน 502
   → งานที่บีบสำเร็จ (95%) ถูกรายงานกลับผู้ใช้ว่า "บีบไม่สำเร็จ" ทั้งที่ไฟล์+Drive+โควต้าเดินครบแล้ว
   · **ต้องผ่านลิงก์สั้น `app.perpos.ai/f/<code>`** (`file_links` kind=`pdf`) เสมอ
   · เคสชื่อไฟล์อังกฤษล้วนรอดมาตลอด จึงไม่เจอตอนเทส — **เทสด้วยชื่อไฟล์ไทยยาว ๆ ทุกครั้ง**
10. **ส่งกลับ LINE ต้องอยู่นอก try ของงานบีบ** — เดิม `deliverToLine` อยู่ในบล็อกเดียวกับงานบีบ
    ส่งไม่สำเร็จเลย**เขียนทับสถานะ `completed` → `failed`** ทั้งที่ไฟล์บีบเสร็จแล้ว
    · ตอนนี้ retry 1 ครั้งแล้วปล่อยให้ job คา `completed` (ตามไฟล์เจอย้อนหลังได้)
11. **`supabase-js` encode `download` ซ้ำ** — `createSignedUrl(path, ttl, { download })` ทำให้
    `%E0%B8…` กลายเป็น `%25E0%25B8…` → ผู้ใช้ได้ไฟล์ชื่อ `%E0%B8%AB….pdf`
    · **อย่าส่ง `download` เข้า supabase-js** ให้ต่อ `&download=${encodeURIComponent(name)}` เอง
    · และอย่าใช้ `NextResponse.redirect()` กับ URL นั้น (normalize แล้ว encode ซ้ำอีกชั้น) —
    ส่ง `Location` ดิบด้วย `new Response(null, { status: 302 })`
