...
3. **push Flex card เข้ากลุ่ม LINE ของทีมแอดมิน** (`notify_group_id`) ผ่าน **บอท PERPOS (@perpos)**
   ...
   · **ปิดแจ้งเตือนได้ที่ตั้งค่าบอท** (`tmc_bot_settings.notify_escalations = false`) — บอทยังส่งต่อให้คน
   (เข้าโหมด `human` + เขียนเคส) แค่ไม่ push การ์ดเข้ากลุ่ม · **ปิดแจ้งเตือน ≠ ปลดผูกกลุ่ม**
   กลุ่มเดิมยังใช้แท็ก `@perpos` เพิ่มความรู้/สั่งกฎได้เหมือนเดิม
...
0. AI แยกก่อนว่าในข้อความมีอะไรบ้าง — **ข้อมูล (fact)** / **คำสั่งวิธีพูด (rule)** /
   **ตัวอย่างคำตอบที่แอดมินแปะมาให้ดู** (ใช้เป็นบริบทเท่านั้น **ห้ามเขียนลงคลัง** — กับดักเดิมที่เคยทำให้
   คำตอบยาว ๆ ของบอทกลายเป็นบทความ) · ข้อความเดียวแตกได้สูงสุด 5 รายการ ผูกกันด้วย `batch_id`

1. หาบทความเดิมที่ใกล้เคียง **สูงสุด 3 ใบ** ด้วย RAG (เกณฑ์ 0.5) แล้วให้ AI เลือกใบที่ตรงเอง
2. Gemini ตัดสินรายการต่อรายการว่า **แก้ของเดิม** หรือ **เพิ่มเรื่องใหม่**
...
4. กด **"ยืนยันบันทึก"** → เขียนจริง + ฝัง embedding ทันที → บอทเอาไปตอบลูกค้าได้เลย
   ...
   · หลายรายการ → การ์ดรวม (`buildTmcKbBatchFlex`) ปุ่ม **"ยืนยันทั้งหมด"** (`tmckb:okall:<batchId>`) /
   **"ยกเลิก"** (`tmckb:noall:<batchId>`) · รายการที่ระบบไม่รับ (กฎที่ขัด invariant / กฎเต็ม 20 ข้อ)
   **ต้องโชว์ในการ์ดเสมอ ห้ามหายเงียบ**
...
**รหัสห้อง ↔ ชื่อที่ลูกค้าเห็น** (อยู่ใน `RENTABLE_VILLAS` · ชื่อสาธารณะมาจาก [`lib/tmc/villa-naming.ts`](../apps/perpos/src/lib/tmc/villa-naming.ts))

| รหัส | ชื่อที่ลูกค้าเห็น            | หมายเหตุ                                                  |
| ---- | ---------------------------- | --------------------------------------------------------- |
| TMC7 | Thammachat Grand (5 ห้องนอน) | —                                                         |
| TMC1 | Thammachat Villa (4 ห้องนอน) | —                                                         |
| TMC5 | Thammachat Villa (4 ห้องนอน) | หลังที่รับสัตว์เลี้ยง — **ชื่อเดียวกับหลังอื่นโดยตั้งใจ** |

> **ห้ามบอกลูกค้าว่าบ้าน 4 ห้องนอนแยกเป็นหลังธรรมดา/หลัง Pet Friendly** (เจ้าของสั่ง 2026-07-31 —
> ลูกค้าที่ไม่ได้พาสัตว์เลี้ยงจะรู้สึกว่าเคยมีหมาแมวมานอนแล้วไม่เอาหลังนั้น) · ด่านที่ใช้:
> `publicVillaName()` (ชื่อทุกจุดที่ส่งถึงลูกค้า — availability + ส่งรูป) · บล็อกห้องว่างรวมบ้าน 4 ห้องนอน
> เป็นบรรทัดเดียว "ว่าง N หลัง" + บรรทัด **ข้อมูลภายใน** บอกว่าหลังรับสัตว์เลี้ยงว่างไหม (ใช้ตอบเฉพาะตอนลูกค้าถามเอง)
> · กฎประจำตัวข้อ 25/26 คุมอีกชั้น
