...
seed_requests AS (
  ...
  SELECT
    ...
    (SELECT trim(concat_ws(' ', coalesce(r.prefix,''), coalesce(r.first_name,''), coalesce(r.last_name,''))) FROM rep r WHERE r.rn = 1 + ((gs - 1) % greatest(1, (SELECT max(cnt) FROM rep))) LIMIT 1),
    (SELECT coalesce(r.rep_code, 'ตัวแทน') FROM rep r WHERE r.rn = 1 + ((gs - 1) % greatest(1, (SELECT max(cnt) FROM rep))) LIMIT 1),
    'submitted',
    1 + (gs % 6),
    'seed',
...
seed_items AS (
  ...
  CROSS JOIN type_one t
  RETURNING poa_request_id
)
UPDATE public.poa_requests pr
SET
  poa_request_type_id = t.id,
  unit_price = round(COALESCE(t.base_price, 0), 2),
  total_price = round(COALESCE(t.base_price, 0) * pr.worker_count, 2)
FROM type_one t
WHERE pr.id IN (SELECT id FROM seed_requests);
