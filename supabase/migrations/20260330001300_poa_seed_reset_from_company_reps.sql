...
reps AS (
  SELECT
    ...
    cr.first_name,
    cr.last_name,
    row_number() OVER (ORDER BY cr.rep_code ASC, cr.id) AS rn,
    count(*) OVER () AS cnt
  FROM public.company_representatives cr
  WHERE cr.rep_code IS NOT NULL
  ORDER BY cr.rep_code ASC, cr.id
  LIMIT 10
),
type_one AS (
...
seed_requests AS (
  ...
  SELECT
    ...
    (SELECT r.profile_id FROM reps r WHERE r.rn = 1 + ((gs - 1) % greatest(1, (SELECT max(cnt) FROM reps))) LIMIT 1),
    (SELECT r.rep_code FROM reps r WHERE r.rn = 1 + ((gs - 1) % greatest(1, (SELECT max(cnt) FROM reps))) LIMIT 1),
    (SELECT trim(concat_ws(' ', nullif(coalesce(r.prefix,''),''), nullif(coalesce(r.first_name,''),''), nullif(coalesce(r.last_name,''),''))) FROM reps r WHERE r.rn = 1 + ((gs - 1) % greatest(1, (SELECT max(cnt) FROM reps))) LIMIT 1),
    (SELECT r.rep_code FROM reps r WHERE r.rn = 1 + ((gs - 1) % greatest(1, (SELECT max(cnt) FROM reps))) LIMIT 1),
    'submitted',
    1 + (gs % 6),
    'seed',
