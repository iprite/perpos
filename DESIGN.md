...
- ใช้กับ **ตัวเลือกที่เห็นทั้งหมดพร้อมกันได้** (ปกติ 2–5 อัน) — ตัวกรอง, สลับมุมมอง/โหมด, **แท็บในหน้า** (§4 ใช้ตัวนี้ตัวเดียว) และ **คำถาม yes/no ในฟอร์ม** (ใช้ `size="form"`)
- **ไม่มี `<RadioGroup>` ในระบบ** — ตัวเลือก 2–3 อย่างใช้ pill ตัวเดียวทั้งแอป ไม่ว่าจะเป็นตัวกรองหรือค่าที่บันทึกลง DB
  (เคยลอง radio แล้วถอดออก 2026-07-30 — สองมาตรฐานในฟอร์มเดียวกันดูไม่เป็นชุดเดียวกัน)
...
| `form`             | `h-8` | **ตัวเลือก yes/no ในฟอร์ม** (จด VAT/ไม่จด, ขึ้นทะเบียน/ไม่ขึ้น) — เตี้ยกว่าช่องกรอกนิดหน่อย |
| `md`               | `h-9` | เป็น **form control ที่อยู่แถวเดียวกับ `Input`/`CustomSelect`** (ต้องสูงเท่ากัน)            |
