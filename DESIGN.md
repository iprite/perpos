...
- ใช้กับ **ตัวเลือกที่เห็นทั้งหมดพร้อมกันได้** (ปกติ 2–5 อัน) — ตัวกรอง, สลับมุมมอง/โหมด, และ **แท็บในหน้า** (§4 ใช้ตัวนี้ตัวเดียว)
- ค่าจาก list ยาว/ไม่จำกัด → `<CustomSelect>` · boolean on/off ล้วนที่ไม่ต้องมี label สองข้าง → toggle/switch

#### SegmentedControl หรือ RadioGroup — เลือกจาก "ผลของการกด" (2026-07-30)

| กด แล้วเกิดอะไร                                                       | ใช้                  |
| --------------------------------------------------------------------- | -------------------- |
| **เปลี่ยนสิ่งที่เห็นทันที** — ตัวกรอง, แท็บ, สลับการ์ด/ตาราง, ฿/%     | `<SegmentedControl>` |
| **เป็นค่าที่ถูกบันทึกลงฐานข้อมูล** — จด VAT / ไม่จด, ขึ้นทะเบียน สปส. | `<RadioGroup>`       |

เหตุผล: pill อ่านเหมือน "สวิตช์โหมด" — พอเอาไปใช้กับ **ข้อเท็จจริงของข้อมูล** ในฟอร์ม
ผู้ใช้เห็นว่าอันที่เข้มคือ "ปุ่มที่กดค้างอยู่" ไม่ใช่ "คำตอบที่บันทึกไว้" · radio สื่อว่ามีกี่ตัวเลือก
และตอนนี้เลือกอะไรอยู่ได้ตรงกว่า + ใส่คำอธิบายต่อตัวเลือกได้ (`hint`)
...
- **ห้ามใช้ checkbox เดี่ยวแทนคำถาม yes/no ที่มีผลทางกฎหมาย/ภาษี** — checkbox ที่ไม่ติ๊ก
  แยกไม่ออกว่า "ตอบว่าไม่" หรือ "ยังไม่ได้กรอก" · ใช้ radio 2 ตัวเลือกแทน
- checkbox ยังใช้ได้กับ "ยืนยันว่าทำแล้ว" (เช่น แจ้งย้ายสถานที่เก็บบัญชีต่อ DBD แล้ว) และตัวเลือกแบบเลือกได้หลายข้อ
- ฟอร์มยาว → แบ่งเป็นกลุ่มด้วย `<SegmentedControl>` เป็นแท็บด้านบน (ต้นแบบ: กล่องแก้ไขลูกค้า acc_firm)
