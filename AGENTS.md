...
📘 **ผู้ช่วยขาย TMC (LINE @tmcvilla — RAG ตอบลูกค้าเรื่องเข้าพัก + ส่งต่อแอดมิน):** อ่าน [`docs/TMC_SALES_BOT_FEATURE.md`](docs/TMC_SALES_BOT_FEATURE.md) — 6 ตาราง `tmc_kb**`/`tmc*chat*_`, หน้า `/[orgSlug]/tmc/sales-bot`, **invariant ที่ห้ามพัง**: @tmcvilla คนละ channel กับบอท PERPOS (`TMC*LINE_CHANNEL*_`↔`/api/line/tmc/webhook`เท่านั้น **ห้ามใช้ปนกัน**) · บอทตอบจากคลังความรู้เท่านั้น ห้ามเดา (prompt บังคับ`[[NO_ANSWER]]`) · เรื่องห้องว่าง/จอง/โอนเงิน/ต่อรองราคา **ต้องส่งต่อคนเสมอ** · embedding ต้อง `gemini-embedding-001`/768 ทั้ง ingestion+query · `tmc_kb_chunks`RLS deny-all +`match_tmc_kb_chunks`บังคับ`p_org_id`· หลัง escalate บอทต้องเงียบตาม`human_mode_minutes` · ราคา/เงื่อนไขอยู่ในคลังความรู้ ห้ามฮาร์ดโค้ด — **2 ทริกเกอร์ส่งต่อ**: ลูกค้าขอคุยกับคน / บอทตอบไม่ได้ → push Flex หาแอดมินทาง LINE ส่วนตัว (บอท PERPOS) แล้วแอดมินคุยต่อใน LINE OA Manager
