...
// เนื้อรายงาน 1 ภาษา (ใช้ทั้งฉบับหลัก = ไทย และฉบับภาษาต้นทาง)
export type MomBody = {
...
export type TranscriptResult = MomBody & {
  language: string; // รหัสภาษาของเสียงต้นทางที่ตรวจพบ (th/en/ja/…)
  speakers: string[]; // ผู้เข้าร่วม / ผู้พูดที่ตรวจพบ (ชื่อใช้ร่วมทุกภาษา)
  // ฉบับภาษาต้นทาง — มีเฉพาะเมื่อเสียงไม่ใช่ภาษาไทย (ฟิลด์หลักด้านบนเป็นไทยเสมอ)
...
3. ภาษาของรายงาน (สำคัญมาก):
   - ฟิลด์หลัก (meeting_title, executive_summary, key_topics, decisions, action_items, recommendations) ให้เขียนเป็น "ภาษาไทย" เสมอ ไม่ว่าเสียงต้นทางจะเป็นภาษาใด — ถ้าเสียงไม่ใช่ไทยให้แปลเป็นไทยอย่างซื่อตรงและเป็นทางการ คงชื่อเฉพาะ ตัวเลข ศัพท์เทคนิค/ชื่อระบบให้ถูกต้อง
   - "language" = รหัสภาษาของเสียงต้นทางที่ตรวจพบ (เช่น "th", "en", "ja")
   - ถ้าเสียงต้นทาง "ไม่ใช่ภาษาไทย" ให้เพิ่ม "source" = รายงานชุดเดียวกันทั้งหมดในภาษาต้นทาง (โครงสร้างฟิลด์เหมือนฟิลด์หลักทุกประการ) เพื่อให้ได้รายงาน 2 ภาษา — ฉบับไทย + ฉบับภาษาต้นทาง
   - ถ้าเสียงต้นทางเป็นภาษาไทยอยู่แล้ว ให้ "source" เป็น null (ไม่ต้องทำซ้ำ)
   - speakers ใช้ชุดเดียว (ชื่อผู้เข้าร่วมเหมือนกันทุกภาษา ไม่ต้องแปล)
4. โครงรายงาน:
...
ถ้าเสียงไม่ใช่ภาษาไทย "source" ต้องเป็น object รายงานฉบับภาษาต้นทาง โครงสร้างเหมือนฟิลด์หลัก เช่น (เสียงอังกฤษ):
{
  "source": {
    "language": "en",
    "meeting_title": "Meeting title in the original language",
    "executive_summary": "Executive summary in the original language",
    "key_topics": [ { "topic": "Topic", "details": "In-depth details in the original language" } ],
    "decisions": [ "Decision agreed by the meeting" ],
    "action_items": [ { "task": "Action starting with a verb", "assignee": "Owner or 'Unassigned'", "deadline": "Due date or ''" } ],
    "recommendations": [ "Analytical recommendation specific to the discussion" ]
  }
}`;
...
async function runJob(jobId: string, orgId: string): Promise<void> {
  ...
  try {
      ...
      job.source === "recall"
        ...
        : ((
            await admin.rpc("consume_stt_quota", {
              ...
              p_job_id: jobId,
              p_source: String(job.source ?? "web"),
            })
    ...
    if (!reserve?.ok) {
      ...
      console.log(
        `[stt-worker] Job ${jobId} quota exceeded (file ${durationSec}s, remain ${reserve?.remaining_seconds}s)`,
      );
      await admin
        .from("assistant_jobs")
        .update({
            ...
            contents: {
              ...
              body: {
                ...
                contents: [
                  ...
                  {
                    type: "text",
                    text: `ไฟล์เสียงของคุณยาว ~${fileMin} นาที แต่โควต้าคงเหลือมีเพียง ${remainMin} นาที`,
                    wrap: true,
                    size: "xs",
                    color: "#6b7280",
                  },
                  {
                    type: "text",
                    text: "ติดต่อแอดมินเพื่อเพิ่มโควต้าครับ 🙏",
                    wrap: true,
                    size: "xs",
                    color: "#94a3b8",
                    margin: "md",
    ...
    if (!finalized || finalized.length === 0) {
      console.warn(
        `[stt-worker] Job ${jobId} no longer 'processing' (likely stuck-swept) — skipping delivery.`,
      );
      return;
    }

    console.log(
      `[stt-worker] Job ${jobId} completed (${transcript.key_topics.length} topics, ${transcript.action_items.length} actions).`,
    );
    ...
    if (reservedSeconds > 0) {
      await admin.rpc("refund_stt_job", { p_job_id: jobId }).then(
        () => undefined,
        () => undefined,
      );
    }
...
async function prepareDriveUpload(
  baseUrl: string,
  secret: string,
  profileId: string,
  refresh: boolean,
  ...
  return j?.ok && j.accessToken && j.folderId
    ? { accessToken: j.accessToken, folderId: j.folderId }
    : null;
}
...
async function saveAudioToDrive(
  ...
  const title = String(
    (transcript as { meeting_title?: string })?.meeting_title || job.file_name || "recording",
  )
    .replace(/[\\/:*?"<>|]/g, " ")
    .trim()
    .slice(0, 80);
...
async function ingestRecallAudio(
  ...
  // 1. bot → recording id
  const botRes = await fetch(`${base}/api/v1/bot/${botId}/`, {
    headers,
  ...
  for (let i = 0; i < 6 && !downloadUrl; i++) {
    const amRes = await fetch(`${base}/api/v1/audio_mixed?recording_id=${recordingId}`, {
      headers,
      signal: AbortSignal.timeout(20_000),
    });
    if (amRes.ok) {
      const am = (await amRes.json()) as {
        results?: { status?: { code?: string }; data?: { download_url?: string } }[];
      };
      const part = am.results?.[0];
      if (part?.status?.code === "done" && part.data?.download_url)
        downloadUrl = part.data.download_url;
    }
  ...
  await admin
    .from("assistant_jobs")
    .update({ audio_url: storagePath, updated_at: new Date().toISOString() })
    .eq("id", String(job.id));
...
async function uploadToGeminiFiles(
  ...
  if (!startRes.ok) {
    throw new Error(
      `Gemini Files upload (start) failed: ${startRes.status} - ${await startRes.text()}`,
    );
  }
  ...
  if (!uploadRes.ok) {
    throw new Error(
      `Gemini Files upload (finalize) failed: ${uploadRes.status} - ${await uploadRes.text()}`,
    );
  }
...
function parseGeminiUsage(
  ...
  if (Array.isArray(meta.promptTokensDetails)) {
    const a = meta.promptTokensDetails.find(
      (d) => String(d?.modality ?? "").toUpperCase() === "AUDIO",
    );
...
// แกะเนื้อรายงาน 1 ภาษา (6 ฟิลด์หลัก) จาก record — ใช้ซ้ำกับฉบับภาษาต้นทาง (source)
function parseMomBody(o: Record<string, unknown>): MomBody {
  return {
    meeting_title: String(o.meeting_title ?? ""),
    executive_summary: String(o.executive_summary ?? ""),
    key_topics: Array.isArray(o.key_topics)
      ? (o.key_topics as unknown[]).map((e) => {
          const k = (e ?? {}) as Record<string, unknown>;
          return { topic: String(k.topic ?? ""), details: String(k.details ?? "") };
        })
      : [],
    decisions: Array.isArray(o.decisions)
      ? (o.decisions as unknown[]).map((x) => String(x)).filter((s) => s.trim().length > 0)
      : [],
    action_items: Array.isArray(o.action_items)
      ? (o.action_items as unknown[]).map((e) => {
          const a = (e ?? {}) as Record<string, unknown>;
          return {
            task: String(a.task ?? ""),
            assignee: String(a.assignee ?? "ไม่ระบุ"),
            deadline: String(a.deadline ?? ""),
          };
        })
      : [],
    recommendations: Array.isArray(o.recommendations)
      ? (o.recommendations as unknown[]).map((x) => String(x)).filter((s) => s.trim().length > 0)
      : [],
  };
}
...
function parseTranscript(text: string): TranscriptResult {
  ...
  // ฉบับภาษาต้นทาง — parse เฉพาะเมื่อ Gemini ส่ง source กลับมา (เสียงไม่ใช่ไทย)
  const srcRaw = parsed.source;
  let source: TranscriptResult["source"] = null;
  if (srcRaw && typeof srcRaw === "object" && !Array.isArray(srcRaw)) {
    const s = parseMomBody(srcRaw as Record<string, unknown>);
    // ต้องมีเนื้อหาจริงจึงจะถือว่ามีฉบับภาษาต้นทาง (กัน source ว่าง/ผิดรูป)
    if (s.executive_summary || s.key_topics.length > 0 || s.action_items.length > 0) {
      source = { language: String((srcRaw as Record<string, unknown>).language ?? ""), ...s };
    }
  }
...
function buildTranscriptText(t: TranscriptResult): string {
  ...
  if (t.action_items.length) {
    ...
    t.action_items.forEach((a) =>
      parts.push(
        `- ${a.task}${a.assignee ? ` (ผู้รับผิดชอบ: ${a.assignee})` : ""}${a.deadline ? ` [กำหนด: ${a.deadline}]` : ""}`,
      ),
    );
    parts.push("");
  }
