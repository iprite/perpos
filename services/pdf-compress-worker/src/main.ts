...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      console.error(
        "[pdf-compress-worker] compress failed:",
...
app.post("/compress", express.json(), async (req, res) => {
  ...
  try {
    ...
    console.error(
      "[pdf-compress-worker] bucket compress failed:",
      e instanceof Error ? e.message : String(e),
    );
    Sentry.captureException(e);
...
app.post("/process", express.json(), (req, res) => {
  ...
  processJob(jobId, orgId).catch(async (e) => {
    console.error("[pdf-compress-worker] job failed:", e instanceof Error ? e.message : String(e));
    Sentry.captureException(e);
