...
import { compressPdf, rasterizePdf, UserFacingError, PDF_BUCKET } from "./pdf.service";
...
export async function processJob(jobId: string, orgId: string): Promise<void> {
  try {
    ...
    console.error(
      `[pdf-worker] unhandled error job ${jobId}:`,
...
async function runJob(jobId: string, orgId: string): Promise<void> {
  ...
  // โหมดเข้ม (pass 2): pdf_meta.mode='rasterize' — ดึงไฟล์ pass 1 จาก bucket (ไม่โหลด LINE ซ้ำ)
  const meta = (job.pdf_meta ?? {}) as { mode?: string; src_path?: string };
  const isRaster = meta.mode === "rasterize";

  try {
    // 1. หาไฟล์ต้นฉบับ
    let bytes: Buffer;
    if (isRaster) {
      const srcPath = String(meta.src_path ?? "");
      if (!srcPath) throw new Error("rasterize job missing src_path");
      const { data, error: dlErr } = await admin.storage.from(PDF_BUCKET).download(srcPath);
      if (dlErr || !data)
        throw new Error(`download ${srcPath} failed: ${dlErr?.message ?? "no data"}`);
      bytes = Buffer.from(await data.arrayBuffer());
    } else if (job.source === "line" && job.line_message_id) {
      // งาน LINE โหลดเองจาก content API (เก็บ webhook ให้เร็ว)
      bytes = await downloadLineContent(String(job.line_message_id));
    } else {
      throw new Error(`unsupported source for pdf job: ${job.source}`);
    }

    // 2. บีบ (python) — surgical หรือ rasterize · โยน UserFacingError ถ้าเกินเพดาน/ไฟล์เสีย
    const result = isRaster ? await rasterizePdf(bytes) : await compressPdf(bytes);
    ...
    //    surgical: คิดเฉพาะเมื่อบีบได้ถึงเกณฑ์ (default 30%) — ต่ำกว่านั้น = ส่งไฟล์ให้ฟรี
    //    rasterize (โหมดเข้ม): คิดเสมอเมื่อเล็กลง (ผู้ใช้ยืนยันเองว่าต้องการบีบเข้ม)
    const minGain =
      Number(process.env.PDF_MIN_GAIN_RATIO) >= 0 ? Number(process.env.PDF_MIN_GAIN_RATIO) : 0.3;
    const billable = isRaster ? !result.noGain : !result.noGain && result.ratio >= minGain;
    let reserved = false;
    if (billable) {
      const reserve = (
        await admin.rpc("consume_pdf_quota", {
      ...
      if (!reserve?.ok) {
        ...
        console.log(
          `[pdf-worker] job ${jobId} quota exceeded (file ${result.pages}p, remain ${remain}p)`,
        );
        await admin
          .from("assistant_jobs")
          .update({
    ...
    try {
      ...
      if (reserved)
        await admin.rpc("refund_pdf_job", { p_job_id: jobId }).then(
          () => undefined,
          () => undefined,
        );
      throw postErr;
    ...
    const userMsg =
      err instanceof UserFacingError ? err.message : "บีบไฟล์ไม่สำเร็จ กรุณาลองส่งไฟล์ใหม่อีกครั้ง";
    await admin
      .from("assistant_jobs")
