...
export function CrmRemindersModal({
  open,
  onClose,
  tasks,
  customerNameById,
  ...
  const overdue = tasks.filter((t) => t.due_at && crmGroupDueAt(t.due_at) === "overdue");
  const today = tasks.filter((t) => t.due_at && crmGroupDueAt(t.due_at) === "today");
  const next7 = tasks.filter((t) => t.due_at && crmGroupDueAt(t.due_at) === "next7");

  const Section = ({ label, items }: { label: string; items: CrmActivity[] }) => {
            ...
            <div key={t.id} className="flex items-start justify-between gap-3 rounded-lg border border-gray-200 bg-white p-3">
              <div className="min-w-0">
                ...
                <div className="mt-0.5 text-xs text-gray-600">
                  {(customerNameById[t.customer_id] ?? t.customer_id).trim()}
                </div>
                {t.due_at ? (
