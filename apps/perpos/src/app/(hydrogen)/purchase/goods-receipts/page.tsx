...
const config = PURCHASE_DOC_TYPE_CONFIGS.goods_receipt;

export default async function GoodsReceiptsListPage() {
  const organizations        = await getOrganizationsForCurrentUser();
  const activeOrganizationId = await getActiveOrganizationId();
  const { rows, error }      = await fetchPurchaseDocRows("goods_receipt");
