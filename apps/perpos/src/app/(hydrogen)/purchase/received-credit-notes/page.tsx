...
const config = PURCHASE_DOC_TYPE_CONFIGS.received_credit_note;

export default async function ReceivedCreditNotesListPage() {
  const organizations        = await getOrganizationsForCurrentUser();
  const activeOrganizationId = await getActiveOrganizationId();
  const { rows, error }      = await fetchPurchaseDocRows("received_credit_note");
