...
export default function BillingPage() {
  ...
  const invoicePager = usePagination(stripeInfo?.invoices ?? []);
  ...
  useEffect(() => {
    void (async () => {
      ...
      if (!o) {
        setError("ไม่พบ org ที่เปิดใช้งาน");
        setLoading(false);
        return;
      }
      setOrgId(o);
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (!res.ok) {
          setStripeErr(d.error ?? "เกิดข้อผิดพลาด");
          return;
        }
        setStripeInfo(d);
      } catch {
  ...
  if (loading) return <div className="p-8 text-center text-sm text-gray-400">กำลังโหลด…</div>;
  ...
  async function startCheckout() {
    ...
    try {
      ...
      if (!res.ok || !d.url) {
        const m = d.error ?? "เกิดข้อผิดพลาด";
        setActionErr(m);
        toast.error(m);
        return;
      }
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 p-6">
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="mt-0.5 text-sm text-gray-500">{info.org_name}</p>
          </div>
        </div>

        <div className="text-right">
          <div className="mb-1 text-xs text-gray-500">สถานะการชำระ</div>
          <span
            className={`inline-flex items-center gap-1.5 rounded-full border px-3 py-1 text-sm font-medium ${statusCfg.cls}`}
          >
            {statusCfg.icon}
            {statusCfg.label}
          </span>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
          <div className="flex items-center justify-between border-b border-gray-100 px-6 py-4">
            <div>
              <div className="mb-1 text-xs text-gray-500">แพ็กเกจปัจจุบัน</div>
              <span
                className={`inline-flex items-center rounded-full border px-3 py-1 text-sm font-semibold ${planCls}`}
              >
                {PLAN_LABELS[info.plan_tier]}
              </span>
            </div>
            <div className="text-right">
              <div className="mb-1 text-xs text-gray-500">ราคา/เดือน</div>
          ...
          <div className="grid grid-cols-2 gap-4 px-6 py-4">
            <div>
              ...
              <div className="mt-0.5 text-sm font-medium text-gray-900">
                {fmtDate(info.plan_starts_at)}
              </div>
            </div>
            <div>
              <div className="text-xs text-gray-500">วันหมดอายุ</div>
              <div
                className={`mt-0.5 text-sm font-medium ${info.is_expired ? "text-red-600" : "text-gray-900"}`}
              >
            ...
            {info.notes && (
              <div className="col-span-2">
                ...
                <div className="mt-1 whitespace-pre-wrap text-sm text-gray-700">{info.notes}</div>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
          <div className="border-b border-gray-100 px-6 py-4">
            ...
            <div className="mt-0.5 text-xs text-gray-500">
              ตั้งค่าครั้งแรกเพื่อให้ตัดบัตรอัตโนมัติทุกเดือน
            </div>
          </div>
          <div className="space-y-3 px-6 py-4">
            <div className="flex flex-wrap gap-2">
              {!info.has_stripe_subscription && (
                <Button
                  ...
                  disabled={paying || !orgId || !token || info.monthly_price === null}
                >
              ...
              {info.has_stripe_customer && (
                <Button
                  ...
                  disabled={openingPortal || !orgId || !token}
                >
            ...
            {info.has_stripe_subscription && (
              <div className="text-xs text-gray-400">การยกเลิกแพ็กเกจต้องติดต่อทีมงาน PERPOS</div>
            )}
          </div>
        </div>
        ...
        <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm md:col-span-2">
          <div className="flex items-start justify-between gap-4 border-b border-gray-100 px-6 py-4">
            <div>
              ...
              <div className="mt-0.5 text-xs text-gray-500">
                แสดงรายการใบแจ้งหนี้ล่าสุด (สูงสุด 10 รายการ)
              </div>
            </div>
          ...
          <div className="space-y-4 px-6 py-4">
            {stripeLoading && <div className="text-sm text-gray-400">กำลังโหลดรายการ…</div>}
            {!stripeLoading && stripeErr && <div className="text-sm text-red-600">{stripeErr}</div>}
              ...
              <>
                <div className="grid gap-3 text-sm sm:grid-cols-3">
                  <div className="rounded-lg border border-gray-100 bg-gray-50 px-4 py-3">
                    ...
                    <div className="mt-0.5 text-sm font-semibold text-gray-900">
                      {fmtDate(stripeInfo.current_period_end)}
                    </div>
                  </div>
                  <div className="rounded-lg border border-gray-100 bg-gray-50 px-4 py-3">
                    <div className="text-xs text-gray-500">ตัดรอบล่าสุด</div>
                    <div className="mt-0.5 text-sm font-semibold text-gray-900">
                      {fmtDate(stripeInfo.current_period_start)}
                    </div>
                  </div>
                  <div className="rounded-lg border border-gray-100 bg-gray-50 px-4 py-3">
                    <div className="text-xs text-gray-500">การยกเลิก</div>
                    <div className="mt-0.5 text-sm font-semibold text-gray-900">
                      {stripeInfo.cancel_at_period_end ? "ยกเลิกสิ้นรอบ" : "ปกติ"}
                    </div>
                  </div>
                </div>
                ...
                {stripeInfo.invoices.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    <Table>
                      ...
                      <TableBody>
                        {invoicePager.rows.map((inv) => (
                          <TableRow key={inv.id}>
                            <TableCell>{fmtDate(inv.created_at)}</TableCell>
                            <TableCell>
                              {inv.status ? (
                                <StatusBadge tone={INVOICE_TONE[inv.status] ?? "neutral"}>
                                  {inv.status}
                                </StatusBadge>
                              ) : (
                                "—"
                            ...
                            <TableCell align="right" tabular>
                              {fmtMoneyMinor(inv.amount_paid ?? inv.amount_due, inv.currency)}
                            </TableCell>
                    ...
                    <TablePager pager={invoicePager} unit="ใบเสร็จ" />
                  </div>
                )}
      ...
      {/* Limits */}
      <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
        <div className="border-b border-gray-100 px-6 py-3 text-sm font-semibold text-gray-700">
          ขีดจำกัดการใช้งาน
        </div>
        <div className="divide-y divide-gray-100">
          {[
            ...
            {
              label: "API requests/วัน",
              value: fmtLimit(info.effective_limits.maxApiRequestsPerDay),
            },
