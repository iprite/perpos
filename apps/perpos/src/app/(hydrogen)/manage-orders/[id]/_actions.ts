...
export function useManageOrderActions({
  ...
  const closeOrder = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(withBasePath("/api/orders/close"), {
        method: "POST",
  ...
  const billInstallment = useCallback(async () => {
    ...
    setLoading(true);
    setError(null);
    const res = await fetch(withBasePath("/api/invoices/create-from-order-installment"), {
      method: "POST",
    ...
    if (invoiceId) window.open(withBasePath(`/invoices/${invoiceId}`), "_blank", "noopener,noreferrer");
