...
const EMPTY = { id: '', title: '', body: '', level: 'info' as Level, is_active: true, starts_at: '', ends_at: '' };
...
export default function AnnouncementsPage() {
  ...
  const openEdit = (a: Announcement) => setForm({
    id: a.id, title: a.title, body: a.body, level: a.level, is_active: a.is_active,
  ...
  const save = async () => {
    ...
    try {
      const payload = {
        ...(form.id ? { id: form.id } : {}),
        title: form.title.trim(), body: form.body, level: form.level, is_active: form.is_active,
  ...
  return (
    <AdminPage
      ...
      actions={<Button onClick={openCreate}><Plus className="mr-1.5 h-4 w-4" /> สร้างประกาศ</Button>}
    >
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-gray-300 bg-gray-50 py-16 text-center">
          ...
          <Button className="mt-4" size="sm" onClick={openCreate}><Plus className="mr-1.5 h-4 w-4" /> สร้างประกาศ</Button>
        </div>
      ) : (
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    <span className={`rounded-full border px-2 py-0.5 text-xs font-medium ${LEVEL_META[a.level].cls}`}>{LEVEL_META[a.level].label}</span>
                    {a.is_active
                      ? <span className="rounded-full border border-green-200 bg-green-50 px-2 py-0.5 text-xs text-green-700">กำลังแสดง</span>
                      : <span className="rounded-full border border-gray-200 bg-gray-50 px-2 py-0.5 text-xs text-gray-500">ปิดอยู่</span>}
                ...
                <div className="flex flex-shrink-0 items-center gap-1.5">
                  <Button variant="outline" size="sm" onClick={() => toggleActive(a)}>{a.is_active ? 'ปิด' : 'เปิด'}</Button>
          ...
          {form && (
            <div className="space-y-4">
              <div>
                ...
                <Input id="ann-title" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} placeholder="เช่น ปิดปรับปรุงระบบ" className="mt-1" />
              </div>
              <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <CustomSelect
                    ...
                    options={(Object.keys(LEVEL_META) as Level[]).map((l) => ({ value: l, label: LEVEL_META[l].label }))}
                    className="mt-1"
                  />
                </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <Input id="ann-start" type="datetime-local" value={form.starts_at} onChange={(e) => setForm({ ...form, starts_at: e.target.value })} className="mt-1" />
                </div>
                <div>
                  <Label htmlFor="ann-end">สิ้นสุด (ไม่บังคับ)</Label>
                  <Input id="ann-end" type="datetime-local" value={form.ends_at} onChange={(e) => setForm({ ...form, ends_at: e.target.value })} className="mt-1" />
                </div>
              </div>
            </div>
          )}
