...
type Field = { key: string; label: string; type: FieldType; group: string; help?: string; default: unknown };

const FIELDS: Field[] = [
  { key: 'signups_enabled',            label: 'เปิดรับสมาชิกใหม่ (LINE)',  type: 'bool',   group: 'การเข้าถึงระบบ', default: true,  help: 'ปิดเพื่อหยุด auto-provision ผู้ใช้ใหม่ที่แอด LINE OA' },
  { key: 'maintenance_mode_global',    label: 'โหมดปิดปรับปรุงทั้งระบบ',    type: 'bool',   group: 'การเข้าถึงระบบ', default: false, help: 'แสดงหน้าปิดปรับปรุงกับผู้ใช้ทุกคน (ยกเว้น super admin)' },
  { key: 'assistant_trial_seconds',    label: 'โควต้าทดลองผู้ช่วย AI (วินาที)', type: 'number', group: 'ผู้ช่วย AI',     default: 18000, help: 'โควต้าเริ่มต้นของผู้ใช้ใหม่ — 18000 = 300 นาที' },
  { key: 'assistant_monthly_price_thb',label: 'ราคาผู้ช่วย AI (บาท/เดือน)',  type: 'number', group: 'ผู้ช่วย AI',     default: 99 },
  { key: 'stt_max_upload_mb',          label: 'ขนาดไฟล์เสียงสูงสุด (MB)',    type: 'number', group: 'ผู้ช่วย AI',     default: 200 },
  { key: 'support_contact_url',        label: 'ลิงก์ติดต่อซัพพอร์ต',         type: 'text',   group: 'ทั่วไป',         default: '' },
];
...
export default function AdminSettingsPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch('/api/admin/settings', { headers: { Authorization: `Bearer ${await token()}` } });
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          ...
          <p className="text-xs text-gray-400">
            * ค่าเหล่านี้เก็บใน <code>app_settings</code> — การบังคับใช้ (enforcement) ทยอยต่อสายในแต่ละจุดของแอป
          </p>
        </div>
      )}
    </AdminPage>
