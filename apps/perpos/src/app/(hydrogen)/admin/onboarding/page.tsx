...
export default function OnboardingPage() {
  ...
  // กรอง personal modules (ผู้ช่วย AI/Flow B2C — เช่น stt, pdf_compress) ออกจาก shared org modules (Suite B2B)
  // ใช้ is_personal จาก DB เป็นหลัก (ALL_MODULES hardcoded ไม่ครบ — pdf_compress ไม่อยู่ในนั้น) + personalKeys เป็น fallback
  const personalKeys = ALL_MODULES.filter((m) => m.personal).map((m) => m.key);
  const sharedMods = registryMods.filter(
    (m) => !m.is_specific && !m.is_personal && m.is_active && !personalKeys.includes(m.key),
  );
  ...
  const handleNameChange = (name: string) => {
    ...
    setForm((f) => ({
      ...
      specificModules: f.specificModules.map((sm) => ({
        ...sm,
        moduleSlug:
          sm.moduleSlug === defaultModuleSlug(sm.key, f.slug)
    ...
    return (
      <div className="mx-auto max-w-lg py-16 text-center">
        ...
        <p className="mt-2 text-sm text-gray-500">
          <span className="font-medium text-gray-700">{result.org.name}</span> พร้อมใช้งานแล้วที่{" "}
          <code className="rounded bg-gray-100 px-1.5 py-0.5 text-xs">/{result.org.slug}</code>
        </p>

        <div className="mt-6 space-y-2 rounded-xl border bg-gray-50 p-4 text-left text-sm">
          ...
          <div className="flex justify-between">
            ...
            <span
              className={cn("font-medium", result.ownerInvited ? "text-blue-600" : "text-gray-700")}
            >
              {result.ownerInvited ? "✉️ ส่ง invite แล้ว" : "✅ เพิ่มเป็น member แล้ว"}
            </span>
          </div>
          {Object.entries(result.seededSummary).map(([modKey, counts]) => (
            <div key={modKey} className="border-t pt-2">
              <p className="mb-1 text-xs font-semibold text-gray-600">
                {registryMods.find((m) => m.key === modKey)?.label ??
  ...
  return (
    ...
    >
          ...
          <div className="space-y-5">
            ...
            <div>
              ...
              <div className="mt-1 flex items-center gap-2">
                <span className="text-sm text-gray-400">app.perpos.ai/</span>
                <Input
                  id="org-slug"
                ...
                {!slugChecking && slugAvail === true && (
                  <CheckCircle2 className="h-4 w-4 text-emerald-500" />
                )}
                {!slugChecking && slugAvail === false && (
                  <span className="text-xs text-red-500">ถูกใช้แล้ว</span>
                )}
              </div>
              <p className="mt-1 text-xs text-gray-400">
                ตัวพิมพ์เล็ก a-z, 0-9 และ - เท่านั้น ความยาว 3-50 ตัวอักษร
              </p>
            </div>
          </div>
        )}
          ...
          <div className="space-y-5">
            <div>
              ...
              <p className="mt-0.5 text-sm text-gray-500">
                เลือก shared modules และ specific modules ที่ต้องการเปิดให้องค์กรนี้
              </p>
            </div>
                          ...
                          >
                            <div className="flex items-center justify-between">
                              <span
                                className={cn(
                                  "font-medium",
                                  selected ? "text-amber-700" : "text-gray-800",
                                )}
                              >
                                {mod.label}
                              </span>
                              <div className="flex items-center gap-1.5">
                                {mod.is_builtin ? (
                                  <span className="flex items-center gap-0.5 rounded-full bg-slate-100 px-1.5 py-0.5 text-[9px] font-semibold text-slate-400">
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-amber-600">
                        {menuCount > 0
                          ? `${menuCount} เมนู`
                          : `default slug: ${regMod?.href_slug ?? sm.key}`}
                      </p>
                    </div>
                  ...
                  <div>
                    ...
                    <div className="mt-1 flex items-center gap-2">
                      <span className="text-sm text-gray-400">app.perpos.ai/{form.slug}/</span>
                      <Input
                        id={`slug-${sm.key}`}
                    ...
                    <p className="mt-1 text-xs text-gray-500">
                      URL เต็ม:{" "}
                      <code className="rounded border bg-white px-1 py-0.5 text-xs">
                        app.perpos.ai/{form.slug}/{sm.moduleSlug || "(กรุณาใส่ slug)"}
                      </code>
                    </p>
                  </div>
                </div>
          ...
          <div className="space-y-5">
            ...
            <div className="divide-y rounded-xl border bg-gray-50">
              ...
              <div className="flex items-center justify-between px-4 py-3">
                ...
                <code className="rounded bg-white px-2 py-0.5 font-mono text-sm text-gray-700 shadow-sm">
                  /{form.slug}
                </code>
              </div>
      ...
      <div className="mt-5 flex justify-between">
        <Button variant="outline" onClick={prevStep} disabled={step === 1} className="gap-1">
        ...
        {step < 5 ? (
          ...
          <Button onClick={() => void handleSubmit()} disabled={submitting} className="gap-1">
            {submitting ? (
              <>
