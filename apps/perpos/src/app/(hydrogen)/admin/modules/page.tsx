...
import { ChevronDown, ChevronRight, Monitor, History, LayoutGrid, Sparkles } from "lucide-react";
...
function SidebarPreview({ settings }: { settings: ModuleSetting[] }) {
  const enabledModules = settings.filter((s) => s.is_enabled);
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white shadow-sm">
      <div className="flex items-center gap-2 border-b px-4 py-3">
        ...
        <span className="ml-auto text-xs text-gray-400">ตัวอย่าง menu ที่ user จะเห็น</span>
      </div>
      <div className="p-3">
        {enabledModules.length === 0 ? (
          <p className="py-6 text-center text-xs text-gray-400">ยังไม่มี module เปิดอยู่</p>
        ) : (
...
function ChangeHistory({ entries }: { entries: ChangeLogEntry[] }) {
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white shadow-sm">
      <div className="flex items-center gap-2 border-b px-4 py-3">
        ...
        <span className="text-sm font-medium text-gray-700">ประวัติการเปลี่ยนแปลง</span>
      ...
      {entries.length === 0 ? (
        <p className="py-6 text-center text-xs text-gray-400">ยังไม่มีการเปลี่ยนแปลง</p>
      ) : (
...
export default function AdminModulesPage() {
  ...
  // ── Load settings + history when org changes ──────────────────────────────────
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        setChangeLog((json?.changeLog ?? []) as ChangeLogEntry[]);
    ...
    >
      ...
      {selectedOrgId && !loading && settings.length > 0 && (
        <div className="mt-5 flex gap-6">
          ...
          <div className="min-w-0 flex-1">
            <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white">
              ...
              <div className="grid grid-cols-[28px_1fr_72px_repeat(4,60px)] items-center border-b-2 border-gray-100 bg-gray-50 px-4 py-3 text-xs font-semibold uppercase tracking-wide text-gray-500">
                ...
                {ORG_ROLES.map((r) => (
                  <div key={r} className="text-center" title={ROLE_FULL[r]}>
                    {ROLE_LABEL[r]}
                  </div>
                ))}
              </div>
          ...
          <div className="w-64 shrink-0 space-y-4 xl:w-72">
            <SidebarPreview settings={settings} />
