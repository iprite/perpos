...
import { listDocs, DOC_TYPE_LABEL } from "@/lib/admin/product-docs";

// Server Component — gate SSR (super_admin) + ดึงรายการเอกสารตอน render
export default async function ProductDocsPage() {
  const admin = await requireSuperAdminPage();
  const docs = await listDocs(admin);
    ...
    >
      {docs.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-xl border border-gray-200 bg-white py-16 text-center shadow-sm">
          ...
          <h3 className="text-sm font-medium text-gray-900">ยังไม่มีเอกสาร</h3>
          <p className="mt-1 max-w-sm text-sm text-gray-500">
            สั่งทีม Documentation Factory (หลังบ้าน) ให้ผลิตเอกสาร —
            ผลงานจะถูกเขียนเข้าระบบเป็นฉบับร่าง แล้วมาแก้เล็กน้อย/เผยแพร่/export PDF ที่นี่
          </p>
        </div>
      ) : (
