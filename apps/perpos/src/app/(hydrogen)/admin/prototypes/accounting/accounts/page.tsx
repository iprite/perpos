...
// B2 accounts/page.tsx — ผังบัญชี (chart of accounts) tree
// tree ผ่าน TableCell indent (padding-left ตาม level — ไม่ใช่ grid ปลอม)
// + filter type + AccountDialog (code/name/type/parent) · is_system ลบไม่ได้
//
...
import { AccountTypeBadge, ACCOUNT_TYPE_LABEL } from "../_components/backstage-badges";
...
export default function AccountsPage() {
  ...
  // tree เต็ม → filter (เมื่อ filter จะ flatten ตรง ไม่รักษา indent ของแม่ที่ถูกตัด)
  const rows = useMemo(() => {
    const q = search.trim().toLowerCase();
    const isFiltering = q.length > 0 || typeF.length > 0;
    if (!isFiltering) return flattenTree(accounts);
    // filtering: แสดงเป็น flat list (indent = 0) ตามเงื่อนไข
    return accounts
      .filter((a) => {
