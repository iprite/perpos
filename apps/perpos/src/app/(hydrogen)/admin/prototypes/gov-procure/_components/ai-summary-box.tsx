...
// ai-summary-box.tsx — กล่อง "สรุปด้วย AI" (AI-1 Executive Brief, Mock — spec §5b)
// กดปุ่ม → loading จำลอง ~1.2s → โชว์ brief สด (buildLiveBrief จาก orders ปัจจุบัน) — ตัวเลขตรง KPI เสมอ
// สถาปัตยกรรม: rule คำนวณ signals → AI narrate · guardrail UX: "ตรวจสอบก่อนตัดสินใจ" + สรุปใหม่
// ไม่ยิง API จริง (prototype) · mirror hotel/_components/ai-summary-box.tsx
...
import { buildLiveBrief, type GovProcureAiBrief } from "../_fixtures/ai-mocks";
...
export function AiSummaryBox() {
  ...
  const [brief, setBrief] = useState<GovProcureAiBrief | null>(null);

  // จำลอง latency ของ AI (mock — ไม่ยิง API) เมื่อเข้าสู่สถานะ loading
  // สร้าง brief สดจาก orders ปัจจุบันตอนจบ loading → ตัวเลขตรง KPI ล่าสุดเสมอ (ไม่ drift)
  useEffect(() => {
    if (state !== "loading") return;
    const t = window.setTimeout(() => {
      setBrief(buildLiveBrief(orders, settings.sla_threshold));
      setState("done");
    }, 1200);
    return () => window.clearTimeout(t);
  }, [state, orders, settings.sla_threshold]);
...
function AiResult({ data }: { data: GovProcureAiBrief }) {
