...
// orders/page.tsx — รายการงาน (list) — spec §5 #3
// Table primitives + filter bar (ค้นหา/stage/company/กอง) + footer sum (ยอดเสนอตาม filter)
// row clickable → DetailDialog (ไม่มีคอลัมน์ปุ่ม action) · overdue = OverdueBadge (icon+ข้อความ, P2-a)
// ปุ่ม "+ สร้างงาน" (canWrite) เปิด OrderDialog · empty/loading §5d
// ?new=1 → เปิด create dialog อัตโนมัติ (ลิงก์จาก dashboard)
...
export default function OrdersPage() {
  ...
  return (
    <GovProcureShell
      title="รายการงาน"
      description="ตารางงานจัดซื้อทั้งหมด — ค้นหา กรอง และเปิดดูรายละเอียดรายงาน"
          ...
          <Table>
            ...
            <TableBody>
              {loading ? (
                ...
                filtered.map((o) => {
                  const aging = deriveAgingDays(o, TODAY_DATE);
