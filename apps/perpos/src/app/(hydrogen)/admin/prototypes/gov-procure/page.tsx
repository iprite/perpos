...
// page.tsx — แดชบอร์ด (ภาพรวม) — pattern page ที่ module-reviewer ล็อก (P4a)
// hierarchy ล็อก (spec §5 #1 / P1-d):
//   (1) แถว StatCard KPI (pipeline value / เงินค้างรับ / กำไร realized / กำไร pending)
//   (2) การ์ด "เงินค้างรับ / overdue" เด่น (คลิก → receivables) — persona owner job#1
//   (3) pipeline summary ต่อ 6 stage (คลิก → pipeline)
//   (4) AI brief box (ai-summary-box)
// ทุกตัวเลขคำนวณจาก data-context (rule ล้วน) · tabular · empty/loading states §5d
...
import { STAGE_LABELS, STAGE_TONE } from "./_fixtures/types";

// สีแท่ง mini bar ต่อ tone (utility class จากพาเลตต์ — ไม่ hardcode hex)
...
export default function GovProcureDashboardPage() {
  ...
  return (
    <GovProcureShell
      ...
      description="ภาพรวมพอร์ตงานจัดซื้อ — สุขภาพพอร์ต เงินค้างรับ และงานที่ต้องเร่งจัดการ (ณ 1 ก.ค. 2569)"
    ...
    >
      {!hasOrders ? (
        <EmptyPortfolio canWrite={canWrite} />
      ) : (
        <>
          {/* (1) KPI */}
          ...
          <ReceivablesHighlight recv={recv} />

          {/* (3) pipeline summary */}
          <PipelineSummary byStage={kpi.byStage} maxValue={kpi.maxStageValue} />

          {/* (4) AI brief — สร้างสดจาก orders ปัจจุบัน (ตัวเลขตรง KPI เสมอ) */}
          <AiSummaryBox />
        </>
      )}
    </GovProcureShell>
...
/** (2) การ์ดเงินค้างรับ / overdue — สะดุดตาเมื่อ overdue>0 (tone/icon/ยอดแดง) · คลิก→receivables */
function ReceivablesHighlight({
  recv,
}: {
  recv: ReturnType<typeof receivableSummary>;
}) {
  // ไม่มีเงินค้างเกินกำหนดเลย → good-news (tone บวก) — spec §5d Receivables empty
  if (recv.overdueCount === 0) {
    return (
      <Link
        href="/admin/prototypes/gov-procure/receivables"
  ...
  // มี overdue → เด่น (พื้นแดงอ่อน + icon + ยอดแดง) + top 3 งาน
    ...
    <div className="rounded-xl border border-red-200 bg-red-50/70 p-4 sm:p-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <Button variant="outline" size="sm" asChild>
          <Link href="/admin/prototypes/gov-procure/receivables">
  ...
  return (
    <div>
      <div className="mb-2.5 flex items-center justify-between px-1">
        ...
        <Link
          href="/admin/prototypes/gov-procure/pipeline"
...
function EmptyPortfolio({ canWrite }: { canWrite: boolean }) {
  return (
