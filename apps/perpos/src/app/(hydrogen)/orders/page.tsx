...
export default function OrdersPage() {
  ...
  const downloadInvoicePdf = useCallback(
    async (invoice: InvoiceLiteRow) => {
      const res = await fetch(withBasePath("/api/invoices/pdf"), {
        method: "POST",
  ...
  const createReceiptFromInvoice = useCallback(async (invoiceId: string) => {
    const res = await fetch(withBasePath("/api/receipts/from-invoice"), {
      method: "POST",
      headers: { "Content-Type": "application/json" },
                ...
                <Button
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      const token = sessionRes.data.session?.access_token;
                      if (!token) throw new Error("ยังไม่ได้เข้าสู่ระบบ");
                      const res = await fetch(withBasePath("/api/orders/start"), {
                        method: "POST",
                      ...
                      try {
                        const lineRes = await fetch(withBasePath("/api/line/employer/send"), {
                          method: "POST",
                          headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
                ...
                <Button
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      const token = sessionRes.data.session?.access_token;
                      if (!token) throw new Error("ยังไม่ได้เข้าสู่ระบบ");
                      const res = await fetch(withBasePath("/api/line/employer/send"), {
                        method: "POST",
                        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
                        body: JSON.stringify({ kind: "order", id: editingId }),
                      });
            ...
            <Button
              onClick={async () => {
                ...
                try {
                  ...
                  const res = await fetch(withBasePath("/api/invoices/confirm-payment"), { method: "POST", body: form });
                  if (!res.ok) {
                    const data = (await res.json().catch(() => ({}))) as { error?: string };
          ...
          <div className="mt-5 flex flex-wrap justify-end gap-2">
            <Button
              onClick={async () => {
                ...
                const res = await fetch(withBasePath("/api/orders/cancel"), { method: "POST", body: form });
                if (!res.ok) {
                  const data = (await res.json().catch(() => ({}))) as { error?: string };
