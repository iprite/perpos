...
function JobStatusBadge({ status }: { status: string | null }) {
  if (!status) return <span className="text-xs text-slate-300">ยังไม่ระบุ</span>;
...
export default function B2gOrdersPage() {
  ...
  const [activeTab, setActiveTab] = useState<"basic" | "finance" | "timeline" | "internal">(
    "basic",
  );
  ...
  // แบ่งหน้าเฉพาะการแสดงผลในตาราง — สรุปยอด/KPI ยังคิดจาก `filtered` ทั้งชุด
  const pager = usePagination(filtered, {
    resetKey: `${filterStatus}|${filterCompany}|${search}`,
  });
    ...
    >
      {loading ? (
        ...
        <>
          ...
          <FilterBar>
            ...
            />
            <FilterClear
              disabled={!search && !filterStatus && !filterCompany}
              onClick={() => {
                setSearch("");
                setFilterStatus("");
                setFilterCompany("");
          ...
          <Table stickyHeader fillViewport>
              ...
              {filtered.length === 0 ? (
                ...
                pager.rows.map((o) => (
                  <TableRow key={o.id} clickable onClick={() => openEdit(o)}>
                    <TableCell align="center" tabular className="text-xs text-slate-400">
                      {o.seq_no ?? "—"}
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell align="right" tabular className="text-xs text-slate-700">
                      {o.price_incl_vat ? `฿${fmt(o.price_incl_vat, 0)}` : "—"}
                    </TableCell>
                    <TableCell align="right" tabular className="text-xs text-slate-600">
                    ...
                    <TableCell align="center" className="text-xs text-slate-500">
                      {fmtDate(o.contract_date)}
                    </TableCell>
                    <TableCell align="center" className="text-xs text-slate-500">
                      {fmtDate(o.receipt_date)}
                    </TableCell>
                    <TableCell align="center">
                      <JobStatusBadge status={o.job_status} />
                    </TableCell>
                  </TableRow>
                ))
              )}
            ...
            {filtered.length > 0 && (
              <TableFooter>
                <TableRow>
                  <TableCell colSpan={4} align="right" className="text-xs text-slate-500">
                  ...
                  <TableCell align="right" tabular className="text-xs">
                    ฿
                    {fmt(
                      filtered.reduce((s, o) => s + n(o.total_cost_89), 0),
                      0,
                    )}
                  </TableCell>
                  <TableCell align="right" tabular className="text-xs text-emerald-700">
                    ฿{fmt(stats.profit, 0)}
                  </TableCell>
                  <TableCell align="right" tabular className="text-xs text-slate-500">
      ...
      <Dialog open={dlgOpen} onOpenChange={setDlgOpen}>
        <DialogContent size="xl">
          ...
          <DialogBody>
            {/* Tab bar */}
            <div className="-mx-1 mb-4 flex gap-1 border-b px-1">
              {(
                [
                  ["basic", "ข้อมูลพื้นฐาน"],
                  ["finance", "การเงิน"],
                ...
                <button
                  ...
                  className={`rounded-t px-3 py-1.5 text-sm transition-colors ${
                    activeTab === key
                      ? "-mb-px border border-b-white bg-white font-medium text-indigo-600"
              ...
              {activeTab === "basic" && (
                <>
                  ...
                  <div>
                    ...
                    <Input
                      id="customer_name"
                      placeholder="เทศบาลเมืองบางแก้ว"
              ...
              {activeTab === "finance" && (
                <>
                  ...
                  <div className="border-t pt-3">
                    <p className="mb-2 text-xs font-medium text-slate-500">วันที่โอนเงินซื้อของ</p>
                    <div className="grid grid-cols-3 gap-3">
                      <div>
                        <Label>Date</Label>
                        <ThaiDatePicker
                          value={form.transfer_date ?? ""}
                          onChange={(v) => setF("transfer_date", v || null)}
                        />
                      </div>
                      <div>
                        <Label>โอนรอบที่ 1</Label>
                        <ThaiDatePicker
                          value={form.transfer_round1 ?? ""}
                          onChange={(v) => setF("transfer_round1", v || null)}
                        />
                      </div>
                      <div>
                        <Label>โอนรอบที่ 2</Label>
                        <ThaiDatePicker
                          value={form.transfer_round2 ?? ""}
                          onChange={(v) => setF("transfer_round2", v || null)}
                        />
                      </div>
                    </div>
                  </div>
                ...
                <>
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div>
                      ...
                      <CustomSelect
                        value={form.customer_change_slip ?? ""}
                        onChange={(v) => setF("customer_change_slip", v || null)}
                        options={SLIP_FORM_OPTS}
                      />
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div>
                      ...
                      <CustomSelect
                        value={form.petty_cash_slip ?? ""}
                        onChange={(v) => setF("petty_cash_slip", v || null)}
                        options={SLIP_FORM_OPTS}
                      />
                    </div>
                  </div>
                ...
                <>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <ThaiDatePicker
                        ...
                        onChange={(v) => setF("contract_date", v || null)}
                      />
                    </div>
                    <div>
                      ...
                      <ThaiDatePicker
                        ...
                        onChange={(v) => setF("payment_order_date", v || null)}
                      />
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <ThaiDatePicker
                        ...
                        onChange={(v) => setF("delivery_date", v || null)}
                      />
                    </div>
                    <div>
                      ...
                      <ThaiDatePicker
                        ...
                        onChange={(v) => setF("receipt_date", v || null)}
                      />
                    </div>
                  </div>
                  <div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <ThaiDatePicker
                        value={form.finance_payment_date ?? ""}
                        onChange={(v) => setF("finance_payment_date", v || null)}
                      />
                    </div>
                    <div>
                      <Label>วันที่จ่าย Support</Label>
                      <ThaiDatePicker
                        value={form.support_payment_date ?? ""}
                        onChange={(v) => setF("support_payment_date", v || null)}
                      />
                    </div>
                  </div>
                  <div>
                    ...
                    <ThaiDatePicker
                      ...
                      onChange={(v) => setF("commission_payment_date", v || null)}
                    />
                  </div>
                  <div>
