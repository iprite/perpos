...
export default function P2pSupplyPage() {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const res = await fetch(`/api/p2p-supply?orgId=${org.id}`, {
    ...
    >
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl border p-6 text-center space-y-4">
          <p className="text-slate-600 font-medium">เชื่อมต่อระบบโมดูล P2P Supply สำเร็จเรียบร้อยแล้ว!</p>
          <div className="max-w-md mx-auto p-4 bg-slate-50 border rounded-lg text-left text-xs font-mono text-slate-500 space-y-1">
            <p>• Module Key: p2p_supply</p>
            <p>• URL Path: /{orgSlug}/p2p-supply</p>
            <p>• API Route: /api/p2p-supply</p>
