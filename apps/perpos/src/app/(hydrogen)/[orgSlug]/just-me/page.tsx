...
export default function JustMePage() {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const res = await fetch(`/api/just-me?orgId=${org.id}`, {
    ...
    <div className="p-4 md:p-6 space-y-5">
      ...
      <div className="flex items-center justify-between flex-wrap gap-2 border-b pb-4">
        <div>
          ...
          <p className="text-sm text-slate-500">โมดูลการทำงานเฉพาะองค์กร (Just Me)</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl border p-6 text-center space-y-4">
          <p className="text-slate-600 font-medium">เชื่อมต่อระบบโมดูล Just Me สำเร็จเรียบร้อยแล้ว!</p>
          <div className="max-w-md mx-auto p-4 bg-slate-50 border rounded-lg text-left text-xs font-mono text-slate-500 space-y-1">
            <p>• Module Key: just_me</p>
            <p>• URL Path: /{orgSlug}/just-me</p>
            <p>• API Route: /api/just-me</p>
