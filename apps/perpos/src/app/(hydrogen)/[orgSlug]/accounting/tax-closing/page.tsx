...
// tax-closing/page.tsx — B4 ภาษี & ปิดงวด (หลังบ้าน, accountant เครื่องมือเต็ม)
//   Tab(overflow-x-auto): PP30(ถ้า VAT) / ภ.ง.ด. (pnd1/3/53) / ปิดงวด
//   + TaxFilingDialog (recompute/mark-filed) + closePeriod/reopenPeriod · ข้อมูลจาก API จริง
...
import type { AccTaxFiling } from "@/lib/accounting/types";
...
export default function TaxClosingPage() {
  ...
  const { taxFilings, periods, orgSettings, loading, closePeriod, reopenPeriod } =
    useAccountingData();
  const vatRegistered = orgSettings?.is_vat_registered ?? false;

  const [tab, setTab] = useState<TaxTab>("pnd");
  ...
  async function handleClose(id: string, label: string) {
    ...
    if (!r.ok) {
      toast.error(r.error ?? "ปิดงวดไม่สำเร็จ");
      return;
    }
  ...
  async function handleReopen(id: string, label: string) {
    ...
    setBusyPeriodId(null);
    if (!r.ok) {
      toast.error(r.error ?? "เปิดงวดไม่สำเร็จ");
      return;
    }
  ...
  if (!canView)
    return (
      <NoAccess title="ภาษี & ปิดงวด" icon={<Landmark className="h-6 w-6" />}>
        หน้าหลังบ้านนี้สำหรับนักบัญชี — บทบาทของคุณไม่มีสิทธิ์ดูภาษี & ปิดงวด
      </NoAccess>
    );
                        ...
                        {canClose ? (
                          p.status === "open" ? (
                            <Button
                              ...
                              variant="outline"
                              disabled={busy}
                              onClick={() => void handleClose(p.id, label)}
                            >
                            ...
                            <Button
                              ...
                              variant="ghost"
                              disabled={busy}
                              onClick={() => void handleReopen(p.id, label)}
                            >
