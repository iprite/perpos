...
const config = PURCHASE_DOC_TYPE_CONFIGS.payment_summary;

export default async function PaymentSummariesListPage() {
  const organizations        = await getOrganizationsForCurrentUser();
  const activeOrganizationId = await getActiveOrganizationId();
  const { rows, error }      = await fetchPurchaseDocRows("payment_summary");
