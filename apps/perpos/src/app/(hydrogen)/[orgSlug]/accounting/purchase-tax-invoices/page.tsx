...
const config = PURCHASE_DOC_TYPE_CONFIGS.purchase_tax_invoice;

export default async function PurchaseTaxInvoicesListPage() {
  const organizations        = await getOrganizationsForCurrentUser();
  const activeOrganizationId = await getActiveOrganizationId();
  const { rows, error }      = await fetchPurchaseDocRows("purchase_tax_invoice");
