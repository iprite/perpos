...
// _employees-client.tsx — มุมมองพนักงาน (client view, mutation จริง)
// initial มาจาก server (lib/hrm/employees) · เพิ่ม/แก้ → fetch /api/hrm/employees (Bearer) → router.refresh()
// filter/search ทำงานในหน่วยความจำ (dataset เล็ก) · row คลิก → แฟ้ม 360° (B4b)
// gate ปุ่มด้วย canWrite จาก role จริง (server prop)
...
import { fmtMoney, fmtNum, fullName } from "../_components/format";
...
export function EmployeesClient({
  ...
  const kpi = useMemo(() => {
    const active = initial.filter((e) => e.status === "active");
    ...
    return { activeCount: active.length, monthlyCost, dailyCount };
  }, [initial]);
  ...
  async function submitEmployee(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const created = await hrmMutate<Employee>("/api/hrm/employees", "POST", {
        orgId,
      ...
      setOpen(false);
      setForm(EMPTY_FORM);
      toast.success(`เพิ่มพนักงาน ${created.first_name} (${created.employee_code}) เรียบร้อย`);
      router.refresh();
    } catch (err) {
        ...
        <TableBody>
          {rows.length === 0 ? (
            <TableEmpty colSpan={7}>
              <div className="flex flex-col items-center justify-center py-10 text-center">
                ...
                <p className="text-sm font-medium text-gray-700">
                  {initial.length === 0 ? "ยังไม่มีพนักงานในระบบ" : "ไม่พบพนักงานตามเงื่อนไข"}
                </p>
                <p className="mt-1 text-sm text-gray-500">
                  {initial.length === 0
                    ? "เริ่มต้นด้วยการเพิ่มพนักงานคนแรก"
                    : "ลองปรับตัวกรอง หรือเพิ่มพนักงานใหม่"}
                </p>
                {canWrite && (
            ...
            rows.map((e) => (
              ...
              >
                <TableCell className="font-mono text-xs text-gray-500">{e.employee_code}</TableCell>
                <TableCell>
