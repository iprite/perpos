...
import { BedDouble, LogIn, ChevronLeft, ChevronRight, AlertCircle, Loader2 } from "lucide-react";
...
interface Booking extends CalBooking {
  created_at: string;
}
  ...
  return (
    <div className="space-y-3">
        ...
        return (
          <div key={type}>
            <div className="mb-1.5 flex items-center gap-2">
              <span
                className={`rounded border px-2 py-0.5 text-xs font-semibold ${TYPE_COLOR[type]}`}
              >
                {type} Room
              </span>
              ...
              {tr.map((room) => {
                ...
                return (
                  <button
                    ...
                    title={room.booking?.guest_name || room.room_number}
                    className={[
                      "h-10 w-12 rounded border text-xs font-medium transition-colors",
                      room.status === "maintenance"
                        ? "cursor-default border-slate-300 bg-slate-200 text-slate-400"
                        : res
      ...
      <div className="flex gap-4 pt-1 text-xs text-slate-400">
        {[
          ["bg-orange-100 border-orange-300", "เข้าพักอยู่"],
          ["bg-yellow-100 border-yellow-300", "จอง"],
          ["bg-white border-slate-200", "ว่าง"],
        ].map(([cls, label]) => (
...
function BookingRow({ booking, onOpen }: { booking: Booking; onOpen: (b: Booking) => void }) {
  const { t } = useLang();
  const room = booking.pms_rooms;
  const total = totalPay(booking.pms_payments);
  const payMethods = booking.pms_payments
    .map((p) => `${getPayLabel(p.method, t)}: ${Number(p.amount).toLocaleString()}`)
    .join(" / ");
  ...
  return (
    <TableRow clickable onClick={() => onOpen(booking)}>
      <TableCell>
        <span
          className={`whitespace-nowrap rounded border px-2 py-0.5 text-xs font-semibold ${TYPE_COLOR[room.room_type]}`}
        >
          {room.room_number}
        </span>
      </TableCell>
      ...
      <TableCell className="text-slate-600">
        ...
        {booking.check_in_time && (
          <span className="ml-1 text-slate-400">{booking.check_in_time}</span>
        )}
      </TableCell>
      <TableCell className="text-slate-600">
        ...
        {booking.nights && (
          <span className="ml-1 text-slate-400">
            ({booking.nights} {t.unit_night})
          </span>
        )}
      </TableCell>
      <TableCell align="right" tabular>
        {total > 0 ? (
          <span title={payMethods} className="font-medium text-slate-800">
...
export default function UsvillaPage() {
  ...
  const loadRooms = useCallback(async () => {
    ...
    const res = await fetch(`/api/usvilla/rooms?orgId=${orgId}`, {
  ...
  }, [orgId, token, loadRooms, loadBookings]);
  ...
  const handleRoomClick = (room: Room) => {
    if (room.booking) {
      setDetailBooking(room.booking as unknown as CalBooking);
    } else {
      setNewOpen(true);
    }
  };
    ...
    >
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          {
            label: `${t.stat_revenue} ${formatThaiDate(date)}`,
            value: todayTotal.toLocaleString("th-TH", { minimumFractionDigits: 2 }),
      ...
      {rooms.length > 0 && (
        <div className="rounded-xl border bg-white p-4">
          <h2 className="mb-3 text-sm font-semibold text-slate-700">{t.room_status_today}</h2>
              ...
              <TableBody>
                {pager.rows.map((b) => (
                  <BookingRow
                    key={b.id}
                    booking={b}
                    onOpen={(bk) => setDetailBooking(bk as unknown as CalBooking)}
                  />
                ))}
              </TableBody>
            </Table>
            <TablePager pager={pager} unit="รายการจอง" />
          </div>
        )}
      </div>
      ...
      <Dialog open={!!checkoutBooking} onOpenChange={(v) => !v && setCheckoutBooking(null)}>
        <DialogContent size="sm">
          <DialogHeader>
            <DialogTitle>{t.dlg_checkout_title}</DialogTitle>
          </DialogHeader>
          <DialogBody>
            <div className="space-y-1 text-sm">
              <p>
                {t.room_label} <strong>{checkoutBooking?.pms_rooms.room_number}</strong>
              </p>
              <p>
                {t.col_guest} <strong>{checkoutBooking?.guest_name}</strong>
              </p>
            </div>
          </DialogBody>
          <DialogFooter>
            <Button variant="outline" onClick={() => setCheckoutBooking(null)}>
              {t.btn_cancel}
            </Button>
            <Button
              onClick={async () => {
                ...
                const res = await fetch(
                  `/api/usvilla/bookings/${checkoutBooking.id}?orgId=${orgId}`,
                  {
                    method: "PATCH",
                    headers: {
      ...
      <Dialog open={!!cancelBooking} onOpenChange={(v) => !v && setCancelBooking(null)}>
        <DialogContent size="sm">
          <DialogHeader>
            <DialogTitle>{t.dlg_cancel_title}</DialogTitle>
          </DialogHeader>
          <DialogBody>
            <p className="text-sm">
              {cancelBooking &&
                t.dlg_cancel_confirm(cancelBooking.guest_name, cancelBooking.pms_rooms.room_number)}
            </p>
          </DialogBody>
          <DialogFooter>
            <Button
