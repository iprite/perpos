...
export function OrderDialog({
  ...
  async function handleSubmit() {
    ...
    // finance-lock: staff ส่งเฉพาะ field พื้นฐาน (API sanitize ตัด finance ให้อีกชั้น)
    ...
    if (canEditFinance) {
      payload.price_incl_vat = numOrNull(form.price_incl_vat);
      payload.cost_price = numOrNull(form.cost_price);
      payload.gross_profit = numOrNull(form.gross_profit);
      payload.net_receivable = numOrNull(form.net_receivable);
      payload.security_deposit = numOrNull(form.security_deposit);
    }
    ...
    try {
      ...
      const msg = (e as Error).message || "บันทึกไม่สำเร็จ";
      setErr(msg);
      toast.error(msg);
    } finally {
      setSaving(false);
