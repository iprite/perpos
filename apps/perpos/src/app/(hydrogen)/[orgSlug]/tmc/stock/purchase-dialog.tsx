...
export function PurchaseDialog({
  ...
  async function handleSave() {
    ...
    if (valid.length === 0) {
      setError("กรุณากรอกรายการสินค้าอย่างน้อย 1 รายการ");
      return;
    }
    if (!accountId) {
    ...
    setSaving(false);
    if (!res.ok) {
      const m = data.error ?? "เกิดข้อผิดพลาด";
      setError(m);
      toast.error(m);
      return;
    }
        ...
        <DialogBody>
              ...
              {!ocrLoading && !ocrError && lines.some((l) => l.name) && (
                <div className="flex items-center gap-2 rounded-lg border border-green-200 bg-green-50 px-4 py-2.5">
                  <span className="flex items-center gap-1 text-sm font-medium text-green-600">
                    <Check className="h-4 w-4" /> อ่านบิลสำเร็จ
                  </span>
                  <span className="text-xs text-gray-500">
                    — ตรวจสอบรายการและหมวดหมู่ด้านล่างก่อนบันทึก
                  </span>
                </div>
              )}
            </div>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <Button variant="ghost" size="sm" onClick={addLine} className="h-7 text-blue-600">
                <Plus className="mr-1 h-3.5 w-3.5" /> เพิ่มรายการ
              </Button>
            </div>
            ...
            <div className="grid grid-cols-[1fr_80px_90px_90px_32px] gap-2 px-1">
              <p className="text-xs font-medium text-gray-400">สินค้า</p>
              <p className="text-xs font-medium text-gray-400">หน่วย</p>
              <p className="text-xs font-medium text-gray-400">จำนวน</p>
              <p className="text-xs font-medium text-gray-400">ราคา/หน่วย</p>
              <span />
            </div>
            ...
            {lines.map((l) => {
              ...
              return (
                <div
                  key={l.id}
                  className="grid grid-cols-[1fr_80px_90px_90px_32px] items-center gap-2"
                >
                  <Input
                    value={l.name}
                  ...
                  <div className="flex items-center justify-end gap-1">
                    {lineTotal > 0 && (
                      <span className="hidden w-16 text-right text-xs tabular-nums text-gray-400 sm:block">
                        {fmt(lineTotal)}
                      </span>
                    )}
                    <Button
            ...
            <div className="flex justify-end border-t pt-1">
              <div className="text-right">
                ...
                <p className="text-xl font-bold tabular-nums text-gray-900">฿{fmt(total)}</p>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-3 border-t pt-4">
            <p className="text-sm font-semibold text-gray-900">บันทึกบัญชี</p>
            ...
            <div className="space-y-1.5">
              ...
              <Input
                ...
                placeholder="เช่น แมคโค สาขาเชียงราย"
              />
            </div>
          </div>
