    ...
    >
      ...
      <Table>
          ...
          {loading ? (
            ...
            entries.map((e) => (
              <TableRow key={e.id} clickable onClick={() => openEdit(e)}>
                ...
                <TableCell className="max-w-[200px] truncate text-gray-800" title={e.description}>
                  {e.description}
                </TableCell>
                <TableCell className="text-xs text-gray-600">{e.company || "—"}</TableCell>
                <TableCell>
                  {e.category ? <StatusBadge tone="info">{e.category}</StatusBadge> : "—"}
                </TableCell>
                ...
                <TableCell align="right" tabular className="text-blue-600">
                  {fmt(e.collected)}
                </TableCell>
                <TableCell
        ...
        {entries.length > 0 && (
          <TableFooter>
            <TableRow>
              <TableCell colSpan={5} className="text-xs text-gray-600">
                รวมทั้งหมด ({total} รายการ)
              </TableCell>
              <TableCell align="right" tabular className="text-red-600">
                {fmt(totals.total_out)}
              </TableCell>
              <TableCell align="right" tabular className="text-green-600">
                {fmt(totals.total_in)}
              </TableCell>
              <TableCell align="right" tabular className="text-blue-600">
                {fmt(totals.total_collected)}
              </TableCell>
              <TableCell />
            </TableRow>
          </TableFooter>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setForm((f) => ({ ...f, amount_out: e.target.value }))}
                  placeholder="0.00"
                />
              </div>
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setForm((f) => ({ ...f, amount_in: e.target.value }))}
                  placeholder="0.00"
                />
              </div>
              <div>
      ...
      <Dialog open={!!deleteId} onOpenChange={() => setDeleteId(null)}>
        <DialogContent size="sm">
          <DialogHeader>
            <DialogTitle>ยืนยันลบรายการ</DialogTitle>
          </DialogHeader>
          <DialogBody>
