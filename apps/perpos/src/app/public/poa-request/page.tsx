...
export default function PublicPoaRequestPage() {
  ...
  React.useEffect(() => {
    Promise.resolve().then(async () => {
      ...
      try {
        const res = await fetch(withBasePath("/api/public/poa-request"), { method: "GET" });
        if (!res.ok) {
  ...
  React.useEffect(() => {
    ...
    const t = window.setTimeout(() => {
      Promise.resolve().then(async () => {
        ...
        try {
          const res = await fetch(withBasePath(`/api/public/employer-suggestions?rep_code=${encodeURIComponent(repCode)}&q=${encodeURIComponent(q)}`), {
            method: "GET",
          });
                ...
                <div className="flex flex-wrap justify-end gap-2">
                  <Button
                    onClick={async () => {
                      ...
                      setBootstrapError(null);
                      try {
                        const res = await fetch(withBasePath("/api/public/poa-request"), {
                          method: "POST",
