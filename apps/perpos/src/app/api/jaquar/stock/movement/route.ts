...
export async function GET(req: NextRequest) {
  ...
  // Flatten joined profile → created_by_name (ADDITIVE field)
  const movements = (data ?? []).map((m: any) => {
    const { profiles: prof, ...rest } = m;
    return {
      ...rest,
      created_by_name: prof?.display_name || prof?.email || null,
    };
  });
...
export async function POST(req: NextRequest) {
  ...
  // Update item — org_id filter เพิ่มเติมสม่ำเสมอกับ route อื่น (defense-in-depth)
