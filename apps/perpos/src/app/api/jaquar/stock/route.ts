...
export async function POST(req: NextRequest) {
  ...
  const { item_code, description, location, amount_starting, import_jaquar, return_borrowed } = body;
...
export async function PUT(req: NextRequest) {
  ...
  const body = await req.json().catch(() => ({}));
  const { id, description, location, amount_starting, import_jaquar, return_borrowed, total_saleable } = body;
  ...
  if (total === undefined) {
    ...
    if (current) {
      const starting = amount_starting !== undefined ? Number(amount_starting) : Number(current.amount_starting);
      const imp = import_jaquar !== undefined ? Number(import_jaquar) : Number(current.import_jaquar);
      const ret = return_borrowed !== undefined ? Number(return_borrowed) : Number(current.return_borrowed);
