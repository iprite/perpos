  ...
  try {
    ...
    for (const r of outcome.results) {
      ...
      const patch: Record<string, unknown> = {
        ...
        // AI เพิ่งเขียนเนื้อหาใหม่ทับ → การ "เปิดอ่าน" ครั้งก่อนใช้ไม่ได้แล้ว
        //
        // ถ้าไม่ล้าง: ผู้ใช้เปิดดูรายการตอนยังเป็น manual (viewed_at ถูกตั้ง) → ปิด →
        // สั่ง AI เติม → AI เขียนทับ → "ยืนยันทั้งชุด (ข้ามรายการเสี่ยง)" จะไม่ข้ามรายการนี้
        // เพราะ viewed_at ยังอยู่ → ประทับ "ผ่านตาคนแล้ว" ให้เนื้อหาที่ไม่มีใครเคยอ่าน
        // แล้วไหลเข้าคลังไปปนงานถัดไป (ช่องที่ B-B1 ตั้งใจปิด)
        viewed_at: null,
      };
