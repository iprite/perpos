...
const REGISTRY: ServiceDef[] = [
  ...
  },
  {
    ...
    stack: 'LINE Platform',
    platform: 'LINE',
    configEnv: ['LINE_MESSAGING_CHANNEL_ACCESS_TOKEN', 'LINE_MESSAGING_CHANNEL_SECRET', 'LINE_LOGIN_CHANNEL_ID'],
  },
  {
    id: 'stripe',
    name: 'Stripe',
...
async function pingHealth(url: string, healthPath: string): Promise<{ status: 'up' | 'down'; latency_ms: number | null }> {
  const ctrl = new AbortController();
  const timer = setTimeout(() => ctrl.abort(), 4000);
  const t0 = Date.now();
  try {
    const res = await fetch(`${url.replace(/\/$/, '')}${healthPath}`, { signal: ctrl.signal, cache: 'no-store' });
...
export async function GET(req: NextRequest) {
  ...
  const envPresent = (keys?: string[]) => (keys ?? []).map((k) => ({ key: k, present: !!process.env[k] }));
  ...
  const services = await Promise.all(
    REGISTRY.map(async (s) => {
      const base = {
        id: s.id, name: s.name, kind: s.kind, purpose: s.purpose, stack: s.stack,
        platform: s.platform, region: s.region ?? null,
      ...
      if (s.kind === 'worker' && s.urlEnv) {
        ...
        if (!url) {
          return { ...base, url_env: s.urlEnv, configured: false, status: 'not_configured' as const, latency_ms: null };
        }
        const h = await pingHealth(url, s.healthPath ?? '/health');
        return { ...base, url_env: s.urlEnv, configured: true, status: h.status, latency_ms: h.latency_ms };
      }
      ...
      if (s.kind === 'scheduler') {
        ...
        const status =
          ageMs == null ? 'unknown' : ageMs <= 5 * 60_000 ? 'up' : ageMs <= 30 * 60_000 ? 'stale' : 'down';
