/**
 * GET /api/admin/stt-billing — ภาพรวมรายได้/สมาชิกแกะเสียง (super admin)
...
type Payment = { id: string; profile_id: string; plan_id: string | null; kind: string; amount: number; currency: string; status: string; created_at: string };
...
export async function GET(req: NextRequest) {
  ...
  const [{ data: pays }, { data: subs }, { data: plans }] = await Promise.all([
    admin.from('stt_payments').select('id, profile_id, plan_id, kind, amount, currency, status, created_at').order('created_at', { ascending: false }).limit(20000),
    admin.from('stt_subscriptions').select('profile_id, plan_id, status, current_period_end').limit(5000),
    admin.from('stt_plans').select('id, name, kind, price, minutes').limit(200),
  ]);

  const payRows = (pays ?? []) as Payment[];
  const planById = new Map((plans ?? []).map((p) => [p.id as string, p as { name: string; price: number; kind: string; minutes: number }]));
  ...
  const mrr = activeSubs.reduce((s, sub) => s + Number(planById.get(String(sub.plan_id))?.price ?? 0), 0);
  ...
  for (const p of succeeded) {
    ...
    e.count += 1; e.revenue += Number(p.amount || 0); byPlan.set(name, e);
  }
  ...
  if (profIds.length) {
    const { data: profs } = await admin.from('profiles').select('id, display_name').in('id', profIds);
    for (const pr of profs ?? []) nameById.set(pr.id as string, (pr.display_name as string) ?? 'ผู้ใช้');
  }
