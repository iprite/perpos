...
export async function GET(req: NextRequest) {
  ...
  // ── Shipping status ──────────────────────────────────────────────────────────
  if (p.get("shipping") === "1") {
    const { data, error } = await admin.from("audit_ship_cursors").select("*").order("destination");
  ...
  let q = admin
    .from("audit_logs")
    .select(
      `id, sequence_no, action, business_action, table_name, record_id,
       org_id, actor_id, on_behalf_of_org_id, diff_keys,
