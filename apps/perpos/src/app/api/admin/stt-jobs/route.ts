...
export async function GET(req: NextRequest) {
  ...
  let q = admin
    .from("assistant_jobs")
    .select(
      "id, profile_id, file_name, mime_type, file_size, duration_seconds, model, status, source, error_message, created_at, updated_at",
    )
  ...
  const ids = Array.from(
    new Set((jobs ?? []).map((j) => j.profile_id).filter(Boolean)),
  ) as string[];
  ...
  if (ids.length) {
    ...
    for (const p of profiles ?? [])
      nameById.set(p.id as string, (p.display_name as string) ?? "ผู้ใช้");
  }
...
export async function POST(req: NextRequest) {
  ...
  // คืนเครดิต: token_refund (idempotent ต่อ job, คืน token ที่ยังค้างทั้งหมดเข้า lot เดิม)
  let refundedTokens = 0;
  if (job.profile_id) {
    const { data: rd, error: rErr } = await admin.rpc("token_refund", { p_job_id: jobId });
    if (rErr) return Err.dbError(rErr);
    refundedTokens = Number((rd as { tokens_refunded?: number } | null)?.tokens_refunded ?? 0);
  }
  ...
  const { error: uErr } = await admin
    ...
    .update({
      status: "failed",
      error_message: "ปิดงานโดยแอดมิน (ค้างเกินกำหนด) + คืนเครดิต",
  ...
  await logAdminAction(req, auth.userId, {
    ...
    targetType: "stt_job",
    targetId: jobId,
    metadata: { profile_id: job.profile_id, refunded_tokens: refundedTokens },
  });

  return ok({ jobId, status: "failed", refunded_tokens: refundedTokens });
}
