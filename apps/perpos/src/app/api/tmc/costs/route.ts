...
import { stayRevenueOf } from "@/lib/tmc/stay-types";
...
export async function GET(req: NextRequest) {
  ...
  const [invRes, finRes, pettyRes, staysRes] = await Promise.all([
    ...
    db
      .from("tmc_stays")
      .select(
        "check_in, room_rate, stay_type, food_amount, drink_amount, mookata_amount, bbq_amount, property_code",
      )
      .eq("org_id", orgId)
      .gte("check_in", from)
      .lte("check_in", to),
  ...
  type PMap = Record<
    string,
    { income: number; opex: number; petty: number; shared_fin: number; shared_petty: number }
  >;
  const byPropMonth: Record<string, PMap> = {};

  function ensure(month: string, prop: string) {
    if (!byPropMonth[month]) byPropMonth[month] = {};
    if (!byPropMonth[month][prop])
      byPropMonth[month][prop] = { income: 0, opex: 0, petty: 0, shared_fin: 0, shared_petty: 0 };
  ...
  const result: MonthRow[] = months.map((month) => {
    ...
    const propRows = activeInvProps.map((inv) => {
      ...
      const cell = byPropMonth[month]?.[code] ?? {
        income: 0,
        opex: 0,
        petty: 0,
        shared_fin: 0,
      ...
      return {
        code,
        investment: Number(inv.investment_amount),
        investor_cost: inv_cost,
        income,
        opex,
        petty,
        shared_alloc: alloc,
