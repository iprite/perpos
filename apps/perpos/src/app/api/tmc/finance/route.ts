...
export async function GET(req: NextRequest) {
  ...
  const accountIds = p.get("accountIds")
    ? p.get("accountIds")!.split(",").filter(Boolean)
    : p.get("accountId")
      ? [p.get("accountId")!]
      : [];
  ...
  // กัน PostgREST ตัดที่ 1,000 แถวเงียบ ๆ (ยอดรวม/สรุป/ไฟล์ Excel ต้องครบ) — เหมือน /api/tmc/petty-cash
  const { data, error } = await q.range(0, 9999);
  if (error) {
    void recordMetric({ orgId, route: "/api/tmc/finance", method: req.method, status: 500, t0 });
...
export async function POST(req: NextRequest) {
  ...
  const { accountId, entryDate, description, category, propertyCode, income, expense, note } =
  ...
  void recordMetric({ orgId, route: "/api/tmc/finance", method: req.method, status: 201, t0 });
