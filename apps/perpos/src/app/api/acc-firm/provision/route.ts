...
export async function GET(req: NextRequest) {
  ...
  const { data: engagement } = await admin
    ...
    .maybeSingle();
  if (!engagement) return NextResponse.json({ error: "ไม่พบ engagement นี้" }, { status: 404 });

  // 2. Get all firm org members with profile info
  const { data: firmMembers } = await admin
    .from("organization_members")
    .select(
      "user_id, role, profiles!organization_members_user_id_fkey(id, display_name, email, avatar_url)",
    )
...
export async function POST(req: NextRequest) {
  ...
  if (!firmOrgId || !clientOrgId || !userId)
    return NextResponse.json(
      { error: "missing firmOrgId, clientOrgId, or userId" },
      { status: 400 },
    );
  ...
  const { data: engagement } = await admin
    ...
    .eq("status", "active")
    .maybeSingle();
  if (!engagement) return NextResponse.json({ error: "ไม่พบ active engagement" }, { status: 404 });
  ...
  const mmRows = grantModules.map((moduleKey) => ({
    ...
    module_role: firmRoleFor(moduleKey),
    is_active: true,
