...
export async function GET(req: NextRequest) {
  ...
  // 2. Fetch invoices with contact names
  const { data: invoices, error: iErr } = await admin
    .from('invoices')
    .select('id, organization_id, invoice_number, issue_date, due_date, status, total_amount, contact_id, contacts!invoices_contact_id_fkey(name)')
  ...
  for (const orgId of clientOrgIds) {
    ...
    summaryMap.set(orgId, { orgId, orgName: o.name, orgSlug: o.slug, draft: 0, open: 0, due_soon: 0, overdue: 0, totalOverdue: 0 });
  }

  for (const inv of invoices ?? []) {
    const org = orgMeta.get(inv.organization_id);
    if (!org) continue;
    const sum = summaryMap.get(inv.organization_id)!;
