...
export async function GET(req: NextRequest) {
  ...
  (cats ?? []).forEach(c => { if (c.category) catMap[c.category] = true; });
...
export async function POST(req: NextRequest) {
  ...
  const { orgId, entry_date, description, company, category, payee,
          amount_out, amount_in, collected, note } = body;
...
export async function PATCH(req: NextRequest) {
  const body = await req.json();
  const { orgId, id, entry_date, description, company, category, payee,
          amount_out, amount_in, collected, note } = body;
