...
export default function UserSettingsPage() {
  ...
  const refreshSettingsProfile = useCallback(async () => {
    ...
    const res = await fetch(withBasePath("/api/settings/profile"), { headers });
  ...
  const refreshNoti = useCallback(async () => {
    const headers = await authHeader();
    const res = await fetch(withBasePath("/api/settings/line-notifications"), { headers });
    const json = await res.json().catch(() => null);
  ...
  const refreshLineQr = useCallback(async () => {
    ...
    try {
      const headers = await authHeader();
      const res = await fetch(withBasePath("/api/line/link-token"), { method: "POST", headers });
      const json = await res.json().catch(() => null);
  ...
  const onSaveProfile = useCallback(async () => {
    ...
    const headers = await authHeader();
    const res = await fetch(withBasePath("/api/settings/profile"), {
      method: "PATCH",
  ...
  const onUploadAvatar = useCallback(
    async (file: File) => {
      ...
      const res = await fetch(withBasePath("/api/settings/avatar/upload"), { method: "POST", headers, body: form });
      const json = await res.json().catch(() => null);
      if (!res.ok) {
  ...
  const onToggleNoti = useCallback(
    async (eventKey: string, enabled: boolean) => {
      ...
      try {
        const headers = await authHeader();
        const res = await fetch(withBasePath("/api/settings/line-notifications"), {
          method: "POST",
          headers: { ...headers, "content-type": "application/json" },
  ...
  const onUnlinkLine = useCallback(async () => {
    ...
    if (!ok) return;
    const headers = await authHeader();
    const res = await fetch(withBasePath("/api/line/unlink"), { method: "POST", headers });
    const json = await res.json().catch(() => null);
    if (!res.ok) {
