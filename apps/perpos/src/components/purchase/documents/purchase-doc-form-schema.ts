...
export const purchaseDocFormSchema = z.object({
  contactId:      z.string().min(1, "กรุณาเลือกผู้จำหน่าย"),
  issueDate:      z.string().min(1, "กรุณาเลือกวันที่"),
  dueDate:        z.string().optional().nullable(),
  withholdingTax: z.string().optional().nullable(),
  notes:          z.string().optional().nullable(),
  refDocId:       z.string().optional().nullable(),
  items:          z.array(itemSchema).min(1, "อย่างน้อย 1 รายการ"),
});

export type PurchaseDocFormValues = z.infer<typeof purchaseDocFormSchema>;

export type DocTotals = { subTotal: number; vatAmount: number; totalAmount: number };

export function computePurchaseDocTotals(
