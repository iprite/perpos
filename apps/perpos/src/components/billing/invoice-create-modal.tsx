...
export function InvoiceCreateModal({
  ...
  const loadItems = React.useCallback(async () => {
    ...
    setItemsError(null);
    try {
      const res = await fetch(withBasePath("/api/invoices/installment-items"), {
        method: "POST",
  ...
  const submit = React.useCallback(async () => {
    ...
    try {
      if (mode === "full") {
        const res = await fetch(withBasePath("/api/invoices/create-from-order-full"), {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ orderId: id }),
        });
        ...
        if (invoiceId) {
          onCreated?.(invoiceId);
          window.open(withBasePath(`/invoices/${invoiceId}`), "_blank", "noopener,noreferrer");
        }
        onClose();
        setLoading(false);
        return;
      ...
      const res = await fetch(withBasePath("/api/invoices/create-from-order-installment-by-items"), {
        method: "POST",
        headers: { "Content-Type": "application/json" },
