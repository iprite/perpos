...
import { showAccountingBackstage } from "./menu-lens";
import {
  isValidThaiTaxId,
  missingSellerIdentity,
  validateTaxDocumentParties,
} from "./tax-identity";
...
describe("tax-identity — กันใบกำกับที่ใช้ไม่ได้ตาม ม.86/4", () => {
  const ok = {
    seller_name: "บริษัท ทดสอบ จำกัด",
    seller_address: "6/15 หมู่ 7 ต.บางเมือง อ.เมือง สมุทรปราการ 10270",
    seller_tax_id: "0115562010487",
    seller_branch: "สำนักงานใหญ่",
    buyer_name: "บริษัท ผู้ซื้อ จำกัด",
    buyer_address: "88 ถนนพระราม 3 กรุงเทพฯ 10120",
    buyer_tax_id: "0105599000001",
  };

  it("ข้อมูลครบ → ผ่าน", () => {
    expect(validateTaxDocumentParties(ok)).toBeNull();
  });

  it("เลขผู้เสียภาษี placeholder/ผิดรูป ไม่ผ่าน", () => {
    expect(isValidThaiTaxId("0000000000000")).toBe(false); // seed placeholder
    expect(isValidThaiTaxId("1111111111111")).toBe(false);
    expect(isValidThaiTaxId("011556201048")).toBe(false); // 12 หลัก
    expect(isValidThaiTaxId(null)).toBe(false);
    expect(isValidThaiTaxId("0115562010487")).toBe(true);
  });

  it("ที่อยู่ตัวอย่างที่ seed ไว้ หลุดออกไปไม่ได้", () => {
    const miss = missingSellerIdentity({
      name: "บริษัท ทดสอบ จำกัด",
      address: "[ตัวอย่าง — แทนที่ด้วยที่อยู่จริงก่อนใช้งาน] เลขที่ 1 ถนนสุขุมวิท",
      tax_id: "0115562010487",
      branch: "สำนักงานใหญ่",
    });
    expect(miss).toContain("ที่อยู่ตามหนังสือรับรอง");
  });

  it("ขาดสาขา (สำนักงานใหญ่/สาขา) ไม่ผ่าน", () => {
    expect(validateTaxDocumentParties({ ...ok, seller_branch: "" })).toMatch(/สำนักงานใหญ่/);
  });

  it("ผู้ซื้อต้องมีชื่อ+ที่อยู่ (ม.86/4 (4))", () => {
    expect(validateTaxDocumentParties({ ...ok, buyer_address: null })).toMatch(/ผู้ซื้อ/);
  });

  it("ผู้ซื้อไม่มีเลขผู้เสียภาษีได้ (รายย่อย) แต่ถ้ากรอกต้องถูกรูป", () => {
    expect(validateTaxDocumentParties({ ...ok, buyer_tax_id: null })).toBeNull();
    expect(validateTaxDocumentParties({ ...ok, buyer_tax_id: "123" })).toMatch(/ผู้ซื้อ/);
  });
});

describe("เมนู sidebar — role lens ฝั่งหลังบ้าน (Phase 2)", () => {
  it("staff ไม่เห็นกลุ่มหลังบ้าน (matrix: ไม่มีสิทธิ์เลย)", () => {
    expect(showAccountingBackstage("staff")).toBe(false);
  });

  it("owner / accountant / viewer ยังเห็น (V/W/V)", () => {
    for (const r of ["owner", "accountant", "viewer"])
      expect(showAccountingBackstage(r)).toBe(true);
  });

  it("super_admin เห็นเสมอ แม้ role จะเป็น staff", () => {
    expect(showAccountingBackstage("staff", true)).toBe(true);
  });

  it("ยังไม่รู้ role (context ยังไม่มา) → ไม่ซ่อน กันเมนูวูบตอนโหลด", () => {
    expect(showAccountingBackstage(null)).toBe(true);
    expect(showAccountingBackstage(undefined)).toBe(true);
  });
});
