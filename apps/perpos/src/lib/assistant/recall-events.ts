...
export function buildBotFlex(
  ...
  const head =
    kind === "fatal"
      ? { bg: "#C43448", title: "❌ เข้าห้องประชุมไม่สำเร็จ" }
      : kind === "over_quota"
        ? { bg: "#3C3B3D", title: "⏱️ บอทออกเพราะครบโควต้า" }
        : { bg: "#3C3B3D", title: "🤖 บอทออกจากห้องประชุมแล้ว" };

  const lines: Record<string, unknown>[] = [
    {
      type: "text",
      text: opts.reason ?? "ดำเนินการเสร็จสิ้น",
      size: "sm",
      wrap: true,
      color: "#3C3B3D",
    },
  ...
  if (kind === "fatal") {
    lines.push({
      type: "text",
      text: "คืนโควต้าให้แล้ว · พิมพ์วางลิงก์ใหม่เพื่อลองอีกครั้งได้เลยครับ 🙏",
      size: "xs",
      wrap: true,
      color: "#9CA3AF",
      margin: "md",
    ...
    lines.push({
      type: "text",
      text: "บันทึกได้เท่าที่โควต้ามี — เติมโควต้าเพื่อบันทึกประชุมเต็มครั้งหน้า",
      size: "xs",
      wrap: true,
      color: "#9CA3AF",
      margin: "md",
  ...
  if (typeof opts.remainMin === "number") {
    lines.push({
      type: "text",
      text: `🤖 โควต้าบอทคงเหลือ ${opts.remainMin} นาที`,
      size: "xs",
      color: "#9CA3AF",
      margin: "sm",
    });
  ...
  return {
    ...
    contents: {
      ...
      header: {
        type: "box",
        layout: "vertical",
        backgroundColor: head.bg,
        paddingAll: "14px",
        contents: [
          { type: "text", text: head.title, color: "#ffffff", weight: "bold", size: "md" },
        ],
      },
...
async function botRemainMin(
  admin: SupabaseClient,
  profileId: string | null,
): Promise<number | undefined> {
  if (!profileId) return undefined;
  const { remainMin } = await getServiceRemaining(admin, profileId, "bot");
  return remainMin;
}

export async function processBotEvent(
  admin: SupabaseClient,
  evt: RecallWebhookEvent,
): Promise<void> {
  ...
  const touch = (patch: Record<string, unknown>) =>
    admin
      .from("assistant_jobs")
      .update({ ...patch, last_sub_code: subCode, updated_at: new Date().toISOString() })
      .eq("id", job.id);
  ...
  switch (code) {
    ...
    case "fatal": {
      ...
      await touch({
        bot_state: "fatal",
        status: "failed",
        error_message: `recall_fatal:${subCode ?? ""}`,
      });
      await pushToProfile(
        admin,
        job.profile_id,
        buildBotFlex("fatal", { reason: subCodeMessage(subCode) }),
      );
      break;
    }
    ...
    case "call_ended": {
      ...
      if (abnormal) {
        ...
        await pushToProfile(
          admin,
          job.profile_id,
          buildBotFlex("ended", { reason: subCodeMessage(subCode), remainMin }),
        );
      }
      break;
    }
    ...
    case "done": {
      ...
      if (!job.recording_started_at) {
        ...
        await pushToProfile(
          admin,
          job.profile_id,
          buildBotFlex("ended", { reason: "ไม่พบการประชุม/เสียงในห้อง จึงไม่คิดโควต้าครับ" }),
        );
        break;
      }
      ...
      const actualSec = joined
        ? Math.max(0, Math.round((new Date(updatedAt).getTime() - joined) / 1000))
        : (job.hold_seconds ?? 0);
      await admin.rpc("settle_bot_quota", { p_job_id: job.id, p_actual_seconds: actualSec }).then(
        () => undefined,
        () => undefined,
      );
      ...
      await touch({
        ...
        recording_url: botId ? `recall:${botId}` : null,
