...
import { acquireSchedulerLease, releaseSchedulerLease } from "./lease";
...
export interface SchedulerResult {
  /** HTTP status ที่ route ควรตอบ (worker ไม่สน) */
  status: number;
  body: Record<string, unknown>;
}

/**
 * ตัวรันงานตามเวลา 1 รอบ — เรียกได้จาก 2 ทาง: worker process (`src/worker/scheduler-worker.ts`,
 * ทางหลักบน VPS) และ `POST /api/assistant/scheduler` (ยิงมือ/สำรอง) · **ห้ามมี Next import ในไฟล์นี้**
 * เพราะ worker ถูก bundle ด้วย esbuild แยกจาก Next
 *
 * กันชนกัน: ทุกรอบต้องได้ lease `scheduler:tick` ก่อน (RPC `scheduler_acquire_lease`) — worker กับ
 * HTTP/cron ยิงพร้อมกันหรือ worker 2 ตัวโดยพลาด = ตัวที่มาทีหลังข้ามรอบ (`skipped: "locked"`)
 * ไม่ใช่รันซ้อน (บั๊กเดิมของ cron 1 นาที: รอบใหม่เริ่มทั้งที่รอบเก่ายังไม่จบ)
 */
export async function runScheduler(opts: { owner: string }): Promise<SchedulerResult> {
  const admin = createAdminClient();
  const gotLease = await acquireSchedulerLease(admin, opts.owner);
  if (!gotLease) return { status: 200, body: { ok: true, skipped: "locked" } };
  try {
    return await runSchedulerTick(admin);
  } finally {
    await releaseSchedulerLease(admin, opts.owner);
  }
}
...
async function runSchedulerTick(
  ...
  if (!accessToken) {
    ...
    return { status: 200, body: { ok: false, error: "LINE token not configured" } };
  }
  ...
  try {
    ...
    return { status: 500, body: { ok: false, error: "scheduler_failed" } };
  }
}
