...
import { listVisibleMetrics, matchByKeyword, suggestMetrics } from "./metrics";
...
/** เวกเตอร์หน่วย (index 0) ใช้ให้ cosine = 1 กับตัวเอง และ 0 กับแกนอื่น */
const UNIT_VEC = (() => {
...
function baseDeps(admin: never, over: Partial<RunMetricResult["metric"]> = {}) {
  return {
    ...
    embedQuestion: async () => ({ values: UNIT_VEC, estimatedTokens: 10 }),
...
describe('askBi — path "ตอบไม่ได้" ต้องแยกสาเหตุให้ขาด', () => {
  ...
  const draftProfit = {
    ...
    // prod embed ครบทุกตัว — fixture ต้องสะท้อนความจริง ไม่งั้นเทสจะไม่เจอ
    // เคส "draft ที่ยังไม่ embed ต้องไม่ถูกเปิดเผย" (fail-safe)
    embedding: UNIT_VEC,
  };
  ...
  /**
   * เปิดเผย metric ร่างต้องผ่าน **ทั้ง** คำที่ตรง **และ** ความหมายที่ใกล้
   *
   * QA รอบ 3: "วันนี้เป็นยังไงบ้าง" ไปจับ "คอมมิชชั่นค้างจ่าย" เพราะ token "ยัง" ตรงกับ
   * คำพ้อง "ยังไม่จ่าย" — keyword อย่างเดียวกันคำหน้าที่สั้น ๆ ในภาษาไทยไม่ได้
   */
  it("คำตรงแต่ความหมายไกล → ไม่เปิดเผย metric ร่าง (กัน false positive)", async () => {
    const admin = makeAdmin();
    ...
    const res = await askBi(
      ...
      noCandidateDeps(admin.client, {
        embedQuestion: async () => ({ values: q, estimatedTokens: 10 }),
        listDraftMetrics: async () => [{ ...draftProfit, embedding: far }],
      }),
    );

    expect(res.status).toBe("no_match");
    expect(res.answer.bullets.join(" ")).not.toContain("ยังไม่ได้ยืนยันนิยาม");
  });

  /** fail-safe: draft ที่ยังไม่ได้ embed ต้องไม่ถูกเปิดเผยด้วย keyword อย่างเดียว */
  it("draft ที่ยังไม่ได้ embed → ไม่เปิดเผย (ไม่ถอยไปใช้ keyword ล้วน)", async () => {
    const admin = makeAdmin();
    const res = await askBi(
      { ...baseInput, question: "กำไรเดือนนี้เท่าไร" },
      noCandidateDeps(admin.client, {
        listDraftMetrics: async () => [{ ...draftProfit, embedding: null }],
      }),
    );
  ...
  it("คำตรง + ความหมายใกล้ → เปิดเผยว่ามีตัวชี้วัดร่างอยู่ (ห้ามรัน)", async () => {
    const admin = makeAdmin();
    const q = new Array(768).fill(0);
    q[0] = 1;
    ...
    const res = await askBi(
      ...
      noCandidateDeps(admin.client, {
        embedQuestion: async () => ({ values: q, estimatedTokens: 10 }),
        listDraftMetrics: async () => [{ ...draftProfit, embedding: [...q] }],
      }),
    );

    expect(res.status).toBe("refused");
    expect(res.answer.bullets.join(" ")).toContain("ยังไม่ได้ยืนยันนิยาม");
...
// ─── 10) ตัดคำไทย — คำถามธรรมชาติต้องจับ metric ได้ (QA รอบ 2) ─────────────

describe("matchByKeyword — ตัดคำไทยด้วย Intl.Segmenter", () => {
  const draftProfit = {
    key: "gov_procure.profit_realized",
    label_th: "กำไรที่รับรู้แล้ว",
    definition_th: "กำไรจากงานที่ปิดแล้ว",
    synonyms: ["กำไร", "กำไรรับรู้", "profit"],
  };
  const others = [
    {
      key: "gov_procure.investor_repayment",
      label_th: "คืนเงินต้นต่อนักลงทุน",
      definition_th: "เงินต้นที่คืนให้นักลงทุน",
      synonyms: ["คืนเงินต้น"],
    },
    {
      key: "gov_procure.pipeline_value_incl_vat",
  ...
  const pool = [draftProfit, ...others];

  it("Intl.Segmenter ตัดคำไทยได้จริงบน runtime นี้ (ถ้าไม่ได้ เทสข้างล่างจะหลอก)", () => {
    const seg = new Intl.Segmenter("th", { granularity: "word" });
    const tokens = Array.from(seg.segment("กำไรเดือนนี้เท่าไร"))
      .filter((s) => s.isWordLike)
      .map((s) => s.segment);
    expect(tokens).toContain("กำไร");
  });

  it.each(["กำไรเดือนนี้เท่าไร", "กำไรสุทธิเท่าไหร่", "ปีนี้กำไรเท่าไร", "กำไรรับรู้เท่าไหร่"])(
    "QA: %s → เจอ profit_realized",
    (q) => {
      expect(matchByKeyword(q, pool, 1)[0]?.key).toBe(draftProfit.key);
    },
  );

  it.each(["อากาศวันนี้เป็นไง", "วันนี้เป็นยังไงบ้าง", "สวัสดีครับ"])(
    "คำถามที่ไม่เกี่ยว: %s → ไม่ match อะไรเลย",
    (q) => {
      expect(matchByKeyword(q, pool)).toEqual([]);
    },
  );

  it("คำถามเรื่องอื่นไปหา metric ของตัวเอง ไม่ข้ามไปกำไร", () => {
    expect(matchByKeyword("ขอดูมูลค่าพอร์ตรวม", pool, 1)[0]?.key).toBe(
      "gov_procure.pipeline_value_incl_vat",
    );
  });
});

// ─── 11) clarify ไม่ควรถามเมื่ออันดับ 1 นำขาด (UX — QA รอบ 2) ──────────────

describe("askBi — อันดับ 1 นำขาด = ตอบเลย ไม่ต้องถามกลับ", () => {
  const runner = { key: "gov_procure.order_count", label_th: "จำนวนใบงานจัดซื้อ" };
  const top = { ...metric, key: runner.key, label_th: runner.label_th, similarity: 0.8 };
  const second = {
    ...metric,
    key: "gov_procure.investor_repayment",
    label_th: "คืนเงินต้น",
  ...
  const deps = (admin: never, gapSecondSimilarity: number, confidence = 0.8) => ({
    ...baseDeps(admin),
    matchMetrics: async () => [top, { ...second, similarity: gapSecondSimilarity }],
    extractIntent: async () => ({
      metric_key: top.key,
      params: { comparison: "none" as const },
      confidence,
      // LLM ขอถามกลับ แต่คะแนนนำขาด → ระบบตอบเลย
      needs_clarify: true,
      clarify_reason: "มีตัวชี้วัดใกล้เคียงหลายตัว",
      usage: { tokenIn: 10, tokenOut: 2, model: "gemini-2.5-flash" },
    }),
    runMetric: async () => ({
      ...runResult(),
      metric: { ...runResult().metric, key: top.key, label_th: top.label_th },
    }),
  });

  it("นำเกิน 0.05 → answered (ไม่ clarify)", async () => {
    const admin = makeAdmin();
    const res = await askBi(
      { ...baseInput, question: "มีงานจัดซื้อกี่ใบ" },
      deps(admin.client, 0.7),
    );
  ...
  /**
   * คะแนนติดกันไม่ใช่เหตุผลพอที่จะถามกลับ **ถ้าสองสัญญาณอิสระเห็นตรงกัน**
   * (retrieval จัดอันดับ 1 + LLM เลือกตัวเดียวกันด้วยความมั่นใจ ≥ 0.6)
   * — คำถามธรรมชาติมักได้คะแนนใกล้กันหลายตัวโดยที่คำตอบที่ถูกยังชัด (QA รอบ 3:
   * "มีงานจัดซื้อกี่ใบ" ต้องกดเลือกก่อนทั้งที่ชัดเจน) · ผู้ใช้ยังเห็นบรรทัดนิยามทุกคำตอบ
   * จึงจับได้ทันทีถ้าระบบเลือกผิดตัว
   */
  it("คะแนนติดกัน (นำ 0.01) แต่ LLM มั่นใจและเลือกตรงกับอันดับ 1 → ตอบเลย", async () => {
    const admin = makeAdmin();
    const res = await askBi(
      { ...baseInput, question: "มีงานจัดซื้อกี่ใบ" },
      deps(admin.client, 0.79),
    );
    expect(res.status).toBe("answered");
    expect(res.clarify).toBeUndefined();
  });

  /** ไม่มั่นใจ = ต้องถามกลับเสมอ (§3.1 ข้อ 4 — ห้ามเดาแล้วตอบเป็นตัวเลข) */
  it("คะแนนติดกัน + LLM ไม่มั่นใจ (0.5) → ยังถามกลับ", async () => {
    const admin = makeAdmin();
    const res = await askBi(
      { ...baseInput, question: "มีงานจัดซื้อกี่ใบ" },
      deps(admin.client, 0.79, 0.5),
    );
    expect(res.status).toBe("clarify");
  });

  it("คู่ฐาน VAT ยังถามกลับเสมอ แม้คะแนนนำขาด (D1 ห้ามข้าม)", async () => {
    const admin = makeAdmin();
    const incl = { ...metric, similarity: 0.9 };
    const excl = {
      ...metric,
      key: "gov_procure.pipeline_value_excl_vat",
      label_th: "มูลค่าพอร์ตรวม (ก่อน VAT)",
    ...
    const res = await askBi(
      { ...baseInput, question: "มูลค่าพอร์ตรวมปีนี้" },
      {
        ...baseDeps(admin.client),
        matchMetrics: async () => [incl, excl],
        extractIntent: async () => ({
          metric_key: incl.key,
          params: { comparison: "none" as const },
          confidence: 0.9,
    ...
    expect(res.status).toBe("clarify");
    expect(res.clarify?.options.map((o) => o.metric_key)).toEqual([incl.key, excl.key]);
  });
});
