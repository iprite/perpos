...
import { Sparkles, Mic, Building2, Shield } from "lucide-react";
...
function ContextToggle({ organizations, activeOrganizationId }: SidebarFooterProps) {
  ...
  const items: Item[] = [];
  if (isSuperAdmin) items.push({ key: "admin", label: "Super", icon: <Shield className="h-3 w-3 shrink-0" />, href: "/admin", active: onAdmin });
  if (activeBiz)    items.push({ key: "biz", label: activeBiz.name, icon: <Building2 className="h-3 w-3 shrink-0" />, href: `/${activeBiz.slug}`, active: onBiz });
  if (hasAssistant) items.push({ key: "assistant", label: "ผู้ช่วย AI", icon: <Mic className="h-3 w-3 shrink-0" />, href: "/assistant", active: onAssistant });
